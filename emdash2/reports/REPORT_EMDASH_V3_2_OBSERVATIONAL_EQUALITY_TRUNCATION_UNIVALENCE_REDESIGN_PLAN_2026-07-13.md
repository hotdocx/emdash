...
Current-Compatibility-Retirement: `REPORT_EMDASH_V3_2_PATH_ACTION_AND_EQUIVALENCE_COMPATIBILITY_RETIREMENT_PLAN_2026-07-19.md` owns forward path-action and legacy-retirement work. Its P4 consumer audit retired the one-layer and dimension-indexed D0 observation families, the uninhabited D0 evidence-property capability and conditional theorem, and their self-only examples. P5 mechanically extracted every remaining D0/D1/decoder owner. P9 supersedes the interim `PathActionRefinement` selection: canonical `path_map_func` action is used directly, comparison-only Nat support is removed, dependent witness action remains direct `eq_apd`, and the isolated Sum experiment is retired pending redesign. P10 supersedes P6 by deleting the extracted compatibility module, its seven clients, and the unused legacy OneCat theorem; P11 supersedes P7 by mapping the collision-free native `NAME_EQ1` identifiers to unsuffixed `NAME` without aliases or semantic changes. Historical rows below remain probe provenance, not claims that those symbols are active. The generic `prop_is_trunc_cat_dim` helper remains native proof support
...
Status: retained living predecessor and promoted-work ledger, still active only for unaffected H0, truncation, dimension, and directed tracks. The completed 2026-07-17 equality-valued overlay supersedes this report for equality/equivalence/direct-univalence/groupoidality/structured-J work, its completed walking-endomorphism child discharges the bounded representative-HIT readiness experiment, and the 2026-07-19 path-action/compatibility plan supersedes its forward registry and D0-retirement conclusions. P9 of that plan also supersedes the dated former-action retention rows; P10 deletes the compatibility module, seven clients, and unused legacy OneCat theorem; P11 makes the native equality-valued symbol surface unsuffixed. The historical 1,694-check Nat-action snapshot, Sum promotions, D0 experiments, and `_EQ1` spellings remain provenance, not the current baseline. The full original computational-foundation endpoint remains partial because future consumer-led former designs, generic directed-HIT and reflector infrastructure, and consistency/stratification/normalization/canonicity/semantic-model metatheory remain open; unrestricted native equality-valued evidence property, unconditional finite-`NCat` object truncation, direct Cat/Grpd equality-valued identity, and one representative directed HIT are no longer blockers.
...
### 2026-07-20 compatibility deletion and namespace correction

P10–P11 of the active cleanup plan also supersede every later claim that the
extracted D0/D1/decoder module, its seven clients, or its two-sided OneCat
theorem remains available. They are deleted; the native one-way ordinary-iso
lift and all practical OneCat/WalkingEnd/Nat results remain. With the legacy
exports gone, the native equality-valued declarations are now unsuffixed after
an exact zero-collision mechanical migration. This report deliberately keeps
the old D0/D1 and `_EQ1` names in chronological rows, but they are neither
current API spellings nor compatibility commitments.
