...
## `SYNTAX-PARITY-1D1` Implementation Result

The bounded correction is focused-green. The existing
`ordinary-functor` expected contract now has one optional recursive
`bodyExpected` field whose type is itself exactly an ordinary-functor
expectation. The resolver does not inspect or decompose the outer target
category to invent that field. When, and only when, the parsed body is a
nested lambda and the caller supplied that field, the existing root-lambda
dispatcher resolves the next level. Every level still calls
`CoreCategoricalProgram.lambda`; consequently the existing contextual
compiler checks that the returned inner functor has the outer target
classifier.

The reviewed exchange/currying source is now exactly equal to its direct
...
The same route is demonstrably recursive rather than a two-level special
case:
...
works only when the expected contract explicitly supplies all three
classifiers. Omitting the immediate `bodyExpected` retains
`UNSUPPORTED_NESTED_ABSTRACTION` at the inner lambda. Supplying one for a
non-lambda body, using the wrong inner mode or annotation, or presenting a
stale expected contract fails with
`INCOMPATIBLE_ABSTRACTION_EXPECTATION`. Incompatible inner targets, foreign
classifiers, and ill-typed bodies remain rejected by the existing
categorical program. The scoped-token implementation is unchanged, so its
existing foreign-term and escaped-slot rejection remains authoritative.

The new focused corpus passes 4/4. The frozen graduation audit and base text
regressions pass 17/17, and root TypeScript typecheck and lint pass. The
long aggregate and browser gates were deliberately not rerun: this change
does not cross a Core, checker, runtime, browser, packaging, or Lambdapi
boundary, and the D009 validation contract requires those gates only when
their boundary changes or the later graduation gate specifically requires
them.

There is no new token, located node, grammar production, parser dependency,
category-expression decomposition, inference heuristic, mathematical owner,
...
final-green 1C1, 1C2A, 1C2B, and 1C3 implementations. Recover the
checkpointed graduation audit, approved D009 review, and focused-green 1D1
recursive ordinary-binder correction. Do not reimplement those rows.
...
- **2026-07-30 — `SYNTAX-PARITY-1D1` implemented and focused-green.** Added
  ...
  categorical lambda owner. The reviewed exchange and a three-level
  recursion witness equal direct TypeScript; missing/stale expectations,
  wrong modes/annotations/targets, foreign classifiers, and ill-typed bodies
