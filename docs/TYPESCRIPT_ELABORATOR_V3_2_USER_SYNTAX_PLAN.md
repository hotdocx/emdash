...
| SYNTAX-PARITY-1B0 | focused-green zero-delta audit at `be7000f`; D002 pending | completed `SYNTAX-PARITY-1A` | Measured the textual `indexOf` seam and split contextual index, independent siblings, and dependent/mixed telescopes into 1B1/1B2/1B3 |
| SYNTAX-PARITY-1B1 | gated on D002 | checkpointed 1B0 proposal | Exact unary `indexOf` route, displayed weakening equality, failures, and one reviewer preset |
