...
| `AGENT-EVAL-12B4` | Reproducible real-agent runs and measured graduation | R8 terminal at receipt integrity; R9 hash audit proposed | The sole staged command emitted only `receipt-integrity`; stderr was not opened. Accepted receipt metadata is green, so R9 proposes one exact minimized-receipt digest command before any code change. |
