...
D-NUH-080, explicitly defer this unqualified observation-normalization gap and
...
and native M/N inputs; nine matrix equations and five explicit complete-arrow
interpretations are adopted, with full fourteen-request reuse and no
universal reselection. One transparent matrix-witness constructor and seven
private signatures add no primitive or rule. Five workflow tests and the
seven-signature/fifteen-concrete-assertion conformance boundary qualify this
stage. Next attach the four derived native exactness witnesses and their
concrete consumers under the [subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).
No output-exactness assumption is supplied; the displayed-exactness flag
remains false.
...
cover, annihilation and inverse laws from original P/Q and native normality;
the window specialization lands in the existing native H(D₀). Thirty-five
definitions and sixteen actual whole consumers add no primitive, rule,
opacity or output-exactness premise. Both central H endpoint comparisons
are now available. Next compare the original connecting transformations
and the surrounding maps through their actual cycle/quotient factors under
the [snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).

NUH-6D2c2 now constructs K(γ)≃K(d̄₀ᴰ) through the original whole K
action on native diagram maps and original kernel inclusion cancellation.
Both inverse data projections and ambient reconstruction after arbitrary
whole maps check. Twenty-three definitions and ten consumers add no
primitive, rule, opacity or normality premise. Next identify that kernel
with the original right H through native normality/cover descent, then
prove positive ∂/δ agreement under the
[snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).

NUH-6D2c1 now derives Q(a)≃Q(dₘᴰ) through original whole row/quotient
descents and carries the original γ to χ=⟨0,d̄₀ᴰ⟩. The proved relation
χ∘φ=γ supplies a native whole arrow-diagram map with endpoint actions
φ and id. Twenty-four definitions and twelve whole consumers retain
original choices and add no primitive, rule or new normality premise.
Next compare the actual kernels, then identify the original right H and
prove positive ∂/δ agreement under the
[snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).

NUH-6D2b now derives Q(α)≃H(A₁) through the original whole Q descents.
The cycle comparison gives v∘α=[0,β_A]; whole uncopairing and the original
Q-units derive both annihilations. Original quotient cancellation proves
both inverse laws, with no new normality or output-equivalence premise.
Fourteen definitions and nine actual whole consumers retain the original
P/Q and check the existing native LES target H. The right homology endpoint
and positive ∂/δ comparison are next in the
[snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).

NUH-6D2a now proves the whole comparison u:Z(A₁)⇒K(c), with a derived
inverse v and fixed-forward ΩAlong(u). Original row/kernel cancellation
proves both inverse laws; both selected inverse projections and their
whole Hom action check. Thirteen transparent definitions and eight consumer assertions
add no primitive, rule, opacity or normality premise. The original P/Q and
row short-exactness capabilities are retained. Next compare α with the
original left boundary and derive Q(α)≃H(A₁), followed by the right
endpoint and positive ∂/δ comparison in the
[snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).

NUH-6D1 now constructs the whole general-snake input of the original
four-row LES window: a=[bₘ,i₀], b=b₀, c=⟨b₁,p₁⟩. Original whole row
reconstruction and the two middle-column zeros derive c∘b∘a=0. Nine
transparent definitions add no primitive or rule; 18 assertions exercise
the whole input, its retained action, original projections, recovery and
first native α/γ consumers. Input formation requires no P/Q, row
short-exactness or normality. Next construct the native universal endpoint
comparisons and compare the positive ∂/δ formulas under the
[snake/LES subplan](TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).
The retained 6C3b observation gap and displayed CAS exactness transport
...
or rule. NUH-6C2a now qualifies the first comparison's cokernel-zero proof
and fixed-forward exactness at K(b), using the original image cover and
derived kernel representatives. NUH-6C2b1 now qualifies the second comparison's
cokernel-zero proof and fixed-forward exactness at K(γ), using two original
whole covers and a corrected native K(b) representative. Its 28 definitions
and 11 passing assertions add no primitive, rule or assumption. NUH-6C2b2a
...
construction and map/input observations now qualify in NUH-6C3a: 13
definitions and 19 assertions, with no new primitive or rule. The larger
comparison/witness observations remain the explicit NUH-6C3b gap. Advance
independent LES/sign work next, as the user permits, then return to that gap
before final qualification. The earlier NUH-5 modular pair/certificate
consumer now passes unchanged at 2GiB with `OCAMLRUNPARAM=o=20`; its actual
public pair alignment and displayed exactness transport remain required.
...
with Terminal views for the concrete window. The frontend constructs the
three actual theorem applications without adding a source assumption or
trust decision. A concrete CAS-window consumer and its exact declaration
dependency closure pass. Six focused tests and 17 affected LP assertions
pass. Whole displayed-diagram transport/coherence remains; the combined
δ-observer/exactness import still exceeds the 2GiB guard and is recorded
separately from the successful modular proof checks.

Current NUH-5N2D milestone (2026-09-15): the bounded native-window driver
validates the original whole CAS adoption/inventory and threads the same M/N
and one immutable source through all three windows. Both zero endpoints and
the nonzero middle pass; repeat use reuses all 111 requests. Four focused
tests pass in 157.71s under a reviewed 240s feature limit, and all nine emitted
LP assertions pass with the usual 90s per-file guard. No active LP owner
changed and no auxiliary endpoint study was resumed. Native exactness, full
H/map assembly, whole-diagram coherence and the later sign comparison remain.

Current NUH-5N2C milestone (2026-09-15): a direct native δ observer uses
the original whole operation with native H endpoints and the original whole
P/Q row predicate at Terminal. The CAS workflow forms all matrix inputs,
adopts native row interpretations and one complete δ agreement, without
legacy H comparisons or standalone point claims. The middle nonzero window
reuses 23 claims and adds eleven computed equations plus three interpretations
(two distinct row predicates and δ). Repeat use reuses all 37 requests.
Twenty-one focused TypeScript tests and 30 affected Lambdapi assertions pass.
The two zero endpoint windows, native exactness realization, complete bounded
diagram/coherence and later snake/LES sign comparison remain required.
...
TypeScript mirrors and a native H realization workflow use those owners
directly. The workflow reuses or computes raw matrix prerequisites and
records one explicit selected-presentation agreement. In the nonsplit
consumer this adds one computed chain equation and one interpretation claim;
no legacy model or formal selected-provider proof is needed. Nine native
tests, four legacy regressions and fifteen affected Lambdapi assertions pass.
The next required work is complete-arrow CAS realization, native δ and
categorical exactness realization. This first point agreement does not by
itself prove whole realization coherence. Details and scoped evidence are in
...
names; old contract terms are rejected. Five LP definitions and one narrower
proof dependency add no primitive or new rule. Eight focused tests and the
concrete nonsplit LP artifact pass through the new separately bounded gate.
The model and interpretation contracts remain explicit; no closed model
is constructed. Next NUH-5B2d2 completes automatic bounded-model adoption
and native exactness observations under the
[model/reifier subplan](TYPESCRIPT_EMDASH_NATIVE_MODEL_REIFIER_PLAN.md).

Current NUH-5B2c2 milestone (2026-09-14): both derived column inputs now
compare categorically with the original raw inputs. Whole source recovery
derives their incoming observations; existing diagram reflection, terminal
uniqueness and fibre inclusion retain each input and selected inverse.
Original whole H gives comparisons to both raw and older retained H objects.
The actual whole δ has the required column endpoints, and its observed
component composes to an arrow between the original raw H objects.
Twenty-three definitions add no primitive or new rule. Three focused
reviewers cover 25 assertions. Next NUH-5B2d migrates TypeScript observations
under the [model/reifier subplan](TYPESCRIPT_EMDASH_NATIVE_MODEL_REIFIER_PLAN.md),
retaining explicit native model/row contracts and the nonsplit adoption gate.
...
on that same comparison, preserving its two endpoints and selected inverse.
The fibre inclusion's base identities and original cell compute through
existing Sigma/base-change owners. The ordinary zero-cone interface retains
its explicit C1 scope. Raw rows/maps also enter as whole constant parameter
families using their existing raw agreements, before K/Q selection.
Thirteen definitions add no primitive, rule, unifier or earlier LP edit.
Nineteen focused assertions pass with unchanged complete warning inventories.
Next assemble the four-row window and its whole chain-zero data, compare
the derived column inputs, and migrate the retained TypeScript observations
under the [model/reifier subplan](TYPESCRIPT_EMDASH_NATIVE_MODEL_REIFIER_PLAN.md).

Current NUH-5B1 milestone (2026-09-14): the supplied native Freyd model
now contains independent whole P/Q, with native whole normality as a separate
enhancement. Its legacy adapter retains the identical whole H and its
original input/point/Hom observations. The model-specialized whole δ and
all three exactness witnesses use the existing generic owners with the
same row families and selected inverses. Twelve definitions add no
primitive, rewrite, unifier or earlier LP edit. Fifteen focused assertions
pass, with identical complete warning inventories to their controls. The
[model/reifier subplan](TYPESCRIPT_EMDASH_NATIVE_MODEL_REIFIER_PLAN.md)
records the interfaces and qualification. Next assemble the concrete raw
window and migrate its TypeScript observations; that retained nonsplit
consumer and later adoption still require qualification.
...
Im(H(p))⇒K(δ) and Im(δ)⇒K(H(i)). Together with C6e2b2, all three actual
native-window comparison maps are invertible in the stated ordinary
Abelian setting. Each original boundary quotient is proved zero from the
original row data, whole K/Q and normality. No output exactness witness,
splitting, representative or extra cover is supplied.

At the source of δ, the original ρ yields a derived cover of K(δ). The
δρ=θ equation makes the covered left cycle a boundary after a second cover. Subtracting its
incoming-row image gives a middle cycle representing the original class
through H(p). At the target of δ, the original q_A yields a cover of
K(H(i)); a second cover supplies a middle boundary representative. Native K lifts
that same map into the original L, and δρ=θ reconstructs the left class.
Original kernel and cover cancellation finish both quotient-zero proofs.

Sixty-one definitions in six new owners add no primitive, runtime rewrite,
unifier or earlier LP edit. δ remains one whole internal transformation
H_C⇒H_A in Functor_cat(B,C), with components obtained by observation.
“Direct” describes its construction by native descents, not a componentwise
replacement or a snake-lemma input.

Next NUH-5 inventories and automates supported model/reifier preparation
while retaining the original whole H, δ and exactness observations in the
nonsplit end-to-end consumer. Classify constructed, supplied and trusted
contracts accurately. Snake/direct/native comparison and final qualification
remain required; these generic proofs do not construct a closed concrete
model or establish a general homology-normalization theorem.

Completed C6e2b2 milestone (2026-09-14): the actual middle comparison
Im(H(i))⇒K(H(p)) now has a derived inverse. The original middle boundary
quotient is proved zero by cancelling the three previously constructed
covers, then the existing inverse criterion certifies the original map.
The original native-window middle comparison has the same qualification
through its literal upper row-triple diagram. No cover or output exactness
witness is supplied as a new assumption.

The necessary whole laws f+0=f, −0=0 and f−0=f are derived through the
original product/coproduct mates. Native restriction retains the original
biproduct injection; the coproduct mate inverse cut gives its whole β law.
This proves q_M(b−β_Mb₀)=q_Mb and identifies the actual homology boundary
on q_Aa with the composite of the three original covers.

Twenty-eight definitions and six proof-time reindexing comparisons add no
primitive or runtime rewrite. The Hom views retain literal paired base
functors. The comparisons extend the existing owner;
the initially proposed extra associativity view was removed by a valid
ablation. The two exactness positions adjacent to δ remain unproved and
are the next C6e2c work. Model/reifier automation, snake comparison and
final qualification remain required.

Completed C6e2b1 milestone (2026-09-14): whole cospan covers now derive
...
A native inverse criterion now constructs an inverse of that same e once
the original quotient of β:A⇒K(g) is proved zero. The original short-exact
row supplies a qualifying instance through its existing β equivalence.
The criterion uses the original Q of a zero arrow, original K/Q cancellation,
and original normality. Seventeen definitions add no primitive, rewrite,
unifier or earlier LP edit.

The three LES boundary-quotient zeros still require proofs from the native
window. The conditional criterion and zero-kernel results do not yet prove
output exactness. Deriving those zeros is the next C6e2b obligation; model/
reifier automation, snake comparison and final qualification remain later.

Completed C6e1 milestone (2026-09-14): the adjacent whole H(i) and H(p)
...
both projection comparisons are proved. Descending coker(r)∘π₁ through
the auxiliary cover forces coker(r)=0. Original normality gives the cover
of r; cancellation of r and the original q_H gives coker(ρ)=0 and the
cover of the existing ρ=q_H∘r.

The [existing native window](../emdash2/emdash3_2_one_cat_native_homology_window_covers.lp)
now instantiates this proof from its original row E0 and supplied Abelian
normality. It does not accept a cover or epicity argument. The general
helper's zero-cokernel premise for p is itself derived from that row.
Original L, r, ρ and H remain the actual terms and endpoints.

Thirty-five definitions also provide whole descent through the original
coimage cover, its reconstruction, inverse-factorization and cancellation
laws. No primitive, rewrite, unifier, old ordinary universal dictionary or
caller naturality square is added. These descent operations are available
for δ once θ∘κ_ρ=0 is proved. That annihilation, δ itself, output exactness,
model/reifier work, snake comparison and final qualification remain open.

Completed C6c3e1 milestone (2026-09-14): the
...
C6c3e2 above applies this auxiliary cover to the original kernel square
and derives the r/ρ covers. Next prove θκ_ρ=0, descend θ to δ, and prove
...
Equal pre/postcomposites have zero composed difference. The direct shear
negative identity is proved equal to the original universal restriction.
No pointwise equality assembly or full additive functor-category capability
is assumed by these results.

The proof first derives id+N=0 from the original whole shear inverse and
projection/bilinearity laws, then proves that original native mates,
pairing, copairing and addition commute with restriction. This transfers
the universal law to the existing N_X, preserving the actual original
product and operation selections. Thirty-eight definitions and eight
proof-time comparisons add no primitive, runtime rule or earlier LP edit.
...
Completed C6c3c1 milestone (2026-09-14): [whole addition](../emdash2/emdash3_2_one_cat_additive_family_bilinearity.lp)
and [subtraction](../emdash2/emdash3_2_one_cat_difference_family_bilinearity.lp)
now preserve composition on both sides. These are whole equations at the
original functors and product choices. They follow from the original
adjunction mates, whole product/coproduct distribution and the existing
whole signed composition laws; no componentwise equality assembly is used.

[Native whole projections](../emdash2/emdash3_2_one_cat_product_family_projection_paths.lp)
recover the original pairing inputs. The
[product-map Hom functor](../emdash2/emdash3_2_product_family_maps.lp)
uses the original postcomposition action and requires no universal-property
argument. Its distribution laws give diagonal naturality at the original
P(W,W) and P(X,X), avoiding replacement by P applied to a differently
presented diagonal family.

Thirty-one definitions and five proof-time comparisons add no primitive,
runtime rule or earlier LP edit. The comparisons cover complete diagonal
Hom action, identity application in Transf Homs, the two native projected
composition views, and unchanged composites under endpoint presentations.
The last comparison repeats both actual arrow operands, preserving the
existing associativity route. Native Sigma annotations remain explicit in
consumers. General Sigma β/η experiments are not part of the promotion.

C6c3c2 above now supplies whole inverses and zero-difference cancellation.
Cover stability/epicity and θκ_ρ=0 remain before δ descent. A whole PreadditiveCategory(Functor_cat(B,C)) and a cover
theorem have not been supplied merely by these bilinearity paths. Output
exactness, concrete model/reifier construction and snake comparison remain
required.

Completed C6c3b milestone (2026-09-14): [whole negation](../emdash2/emdash3_2_one_cat_negative_families.lp)
...
Product_map_func supplies the signed input map for subtraction, retaining
addition's native intermediate Hom. The original fixed-forward inverse remains visible at
components, with both whole inverse laws proved.

C6c3c1 above now proves whole bilinearity. Additive zero/cancellation laws
and the cover stability/epicity and θκ_ρ=0 proofs remain before δ descent.
...
Λ_D(v)=v∘q_D. The [reconstruction owner](../emdash2/emdash3_2_one_cat_cokernel_family_reconstruction.lp)
recovers the original whole descent input and reflects equality through
that same quotient. Descent is independent of the annihilation witness.

The [actual short-exact row maps](../emdash2/emdash3_2_one_cat_short_exact_family_cancellation.lp)
now reconstruct as κ_D∘β=i and γ∘q_F=g, and both whole row cancellation
results follow from their original β/γ evidence and K/Q. The
[canonical map ρ=q_H∘r](../emdash2/emdash3_2_one_cat_homology_cover_maps.lp)
retains the original pullback family and H. Cancellation through q_H is
proved. Its interface accepts the original coherent right-column h and p;
assembling those inputs from the native window remains the next step.

Nineteen definitions, 23 assertions and exact dependency warning comparisons
add no primitive, runtime rule, unifier or nucleus edit. The original
quotient alias lives with the existing image/coimage graph owner. All other
...
C6c1 above now supplies the quotient and row cancellation prerequisites
and the canonical ρ map. Its epicity, θ and δ remain unfinished.
...
e∘a_F=β̄; native inverse mating recovers the original whole β. Reconstruction
through a_F determines e uniquely. The
[exactness interface](../emdash2/emdash3_2_one_cat_categorical_exactness.lp)
is existing OmegaEquivAlong on this actual map, with a whole specialization
to the native zero-cone category. It supplies no exactness inhabitant.

Whole incoming observations precede K/Q selection. The original H boundary,
...
C4b now derives the required annihilation and constructs Im(f)→K(g).
The qualified differential/prewhiskering comparison and three family views
support that actual construction while preserving runtime owners. Exactness
is defined on its actual map; the LES exactness proofs and δ remain required.
...
ordinary inverse equations. Generic functor action restricts this evidence
to whole diagram families and their components, preserving the same inverse.
The [native Abelian package](../emdash2/emdash3_2_one_cat_abelian_adjunctions.lp)
adds whole P/Q and that fixed-a normality over the original AdditiveCategory,
including its whole binary-product functor. Normality is supplied structure,
...
C4b now constructs canonical Im(f)→K(g) from a whole native zero-family input.
...
The completed C2c4 milestone (2026-09-13): the
...
family/shape presentations and their OneCat qualification. C3 now
attaches the existing categorical invertibility evidence to this actual a;
categorical exactness and δ by whole universal descent remain next.
...
The bounded NUH-6C3b2 review is complete and its comparison-normalization gap
is explicitly deferred under D-NUH-080. Complete NUH-7 and NUH-7B without
resuming that investigation. Do not claim the gap solved or reinstate old/new
