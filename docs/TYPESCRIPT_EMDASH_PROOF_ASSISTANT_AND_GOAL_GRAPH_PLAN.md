...
object presentation at unchanged 4/5/0/10. V3 awaits checkpoint and separate
...
proposal v3 are the current uncheckpointed bounded tranche.
...
| `STDLIB-8B` | PathOut/PathInd trusted profile and derived public library | in progress | `PATHOUT-TRUST-BOUNDARY-0A`, root-only `PATHOUT-LIBRARY-FOUNDATION-1B`, and fixed-source `PATHIND-TRUSTED-PROFILE-1C` at semantic checkpoint `a361dc3` are complete and final-proportional-green. Measured dependency-prefix shadowing supersedes internalized PathInd v2 review `2a250fb`. Corrected behavior-free v3 replaces its support rule one-for-one with the stable post-prefix presentation, preserving 4/5/0/10 and every consumer/oracle/denial; it awaits checkpoint and separate review. Transitivity and public presentation remain later rows. |
...
| `D-PA-146` | Supersede v2 implementation authority and freeze corrected non-authorizing internalized PathInd proposal v3 at unchanged 4/5/0/10. | With the correct `Catd_cat(PathOut)` evaluation base, v2's support rule subject-checks, then component admission shows dependency-first `directed.category-hom.decode` and `categorical.mixed-action.functor-classifier-definition` have already shadowed its pre-prefix pattern. V3 replaces that one rule with the exact stable decoded `Functor_cat` object presentation, adds no rule or mathematics, preserves all consumers/oracles/denials, and requires its own checkpoint plus separate review before implementation. Temporary observers are absent and the generic checker diff is empty. |
