...
function runChurchEncodingTests() {
    ...
    const not_func_type = Pi("b_not_param", Icit.Expl, Var("Bool_type"), _b_term => Var("Bool"));
    ...
    assert(areEqual(elabRes.type, not_func_type, baseCtx), "Church Test 8.1: not_hs type check");
...
function runChurchStyleImplicitTests() {
    ...
    defineGlobal("List_hs", List_type_constructor_type, List_type_constructor_val, false, false, true); 
    ...
    assert(areEqual(normalize(elabRes.term, baseCtx), normalize(List_type_constructor_val, baseCtx), baseCtx), "HSI Test 5.2: List_hs value check");

    const nil_func_type = Pi("A_nil_param", Icit.Impl, Type(), A_nil_term => App(Var("List_hs"), A_nil_term, Icit.Expl));
    const nil_func_val_raw = Lam("L_nil_param", Icit.Expl, Type(), L_nil_val_term =>
        Lam("cons_nil_param", Icit.Expl, FH(), _cons_func_term =>  
            Lam("nil_actual_val_param", Icit.Expl, L_nil_val_term, nil_actual_val_term => nil_actual_val_term)
        )
    );
    ...
    assert(areEqual(normalize(elabGlobalNilVal, baseCtx), normalize(nil_func_val_elab_expected, baseCtx), baseCtx), "HSI Test 6.2: nil_hs value check against fully elaborated form");
    ...
    const cons_func_val_raw = Lam("x_cons_val", Icit.Expl, FH(), x_cons_actual_term => 
        Lam("xs_cons_val", Icit.Expl, FH(), xs_cons_actual_term => 
            Lam("L_cons_param", Icit.Expl, Type(), L_cons_val_term => 
                Lam("cons_fn_cons_param", Icit.Expl, FH(), cons_fn_actual_term => 
    ...
    assert(areEqual(normalize(elabGlobalConsVal, baseCtx), normalize(cons_func_val_elab_expected, baseCtx), baseCtx), "HSI Test 7.2: cons_hs value check");
    ...
    const map_func_val_raw = Lam("A_impl_map", Icit.Impl, Type(), A_val_map => 
    ...
    const globalMapHsDef = globalDefs.get("map_hs")!;
    assert(!!globalMapHsDef.value, "HSI Test 8.2a: Global map_hs should have a value");
    assert(areEqual(normalize(check(baseCtx, globalMapHsDef.value!, globalMapHsDef.type), baseCtx), normalize(map_func_val_raw, baseCtx), baseCtx), "HSI Test 8.2: map_hs value check");

    const list1_hs_type = App(Var("List_hs"), Var("Bool"), Icit.Expl);
    const list1_hs_val = App(App(App(Var("cons_hs"), Var("Bool"), Icit.Impl), Var("true_hs"), Icit.Expl), 
                           App(App(App(Var("cons_hs"), Var("Bool"), Icit.Impl), Var("false_hs"), Icit.Expl), 
                               App(Var("nil_hs"), Var("Bool"), Icit.Impl), 
                               Icit.Expl), 
                           Icit.Expl);
    defineGlobal("list1_hs", list1_hs_type, list1_hs_val);
    ...
    assert(areEqual(normalize(elabRes.term, baseCtx), normalize(list1_hs_val, baseCtx), baseCtx), "HSI Test 9.2: list1_hs value check");
                                ...
                                App(s_ten_actual_term, 
                                    App(s_ten_actual_term, 
                                        App(s_ten_actual_term, 
                                            App(s_ten_actual_term, 
                                                App(s_ten_actual_term, 
                                                    App(s_ten_actual_term, z_ten_actual_term, Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl), Icit.Expl)
            )
        )
    );
    ...
    defineGlobal("Eq_hs", Eq_hs_type, Eq_hs_val_raw, false, false, true);
    ...
    assert(areEqual(normalize(elabRes.term, baseCtx), normalize(check(baseCtx, Eq_hs_val_raw, Eq_hs_type), baseCtx), baseCtx), "HSI Test 16.2: Eq_hs value check");
    ...
    elabRes = elaborate(Var("sym_hs"), undefined, baseCtx);
    ...
    const final_expr_type_expected = App(App(App(Var("Eq_hs"), Var("Nat_hs"), Icit.Impl), Var("hundred_hs"), Icit.Expl), Var("hundred_hs"), Icit.Expl);
    ...
    const the_final_expr_term_to_check = App(App(Var("refl_hs"), Var("Nat_hs"), Icit.Impl), Var("hundred_hs"), Icit.Impl);
