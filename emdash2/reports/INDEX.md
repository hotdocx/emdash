...
proves χ∘φ=γ for χ=⟨0,d̄₀ᴰ⟩ and supplies a native whole diagram map
with endpoint actions φ and id. Twenty-four new definitions and twelve
whole consumers retain original choices without a new primitive, rule or
normality premise. Right kernel/H and positive ∂/δ comparisons remain
next in the [subplan](../../docs/TYPESCRIPT_EMDASH_NATIVE_SNAKE_AND_LES_COMPARISON_PLAN.md).
...
Second native snake interior exactness (2026-09-15): the
[new proof](../emdash3_2_one_cat_native_snake_second_exactness.lp) derives
ΩAlong for the actual Im(k₂)⇒K(∂) comparison. Two original whole
...
Direct native observations (2026-09-15):
...
and [retained H comparisons](https://github.com/hotdocx/emdash/blob/df9b4778584121c3ad6ef7575093bd6d84be89e8/emdash2/emdash3_2_commutative_algebra_freyd_retained_column_homology_comparisons.lp)
...
consumers. NUH-3C3 supplies the
...
C6b2b now [constructs the native cartesian comparison and inverse](../emdash3_2_one_cat_kernel_pullback_universality.lp),
with both whole functor laws and original-leg recovery. Its two explicit
...
from original whole short-exact row cancellation, original K/Q universality
and supplied Coim⇒Im normality. The actual q_D is proved zero; the original
image inclusion and existing coimage-to-target factor become invertible.
Whole factor reconstruction explicitly compares the earlier factor with
...
Chapter 31 and its appendices describe this boundary in the checked,
promoted `0.8.3-dev` draft. The worked-example follow-up traces the
...
The later publication continuation adds Book/Code self-references to both
the book and the `0.3.1-dev` overview, removes the overview's 18-page ceiling
without dropping quality checks, and records the explicitly authorized
GitHub delivery in the parent plan.
...
dimension-two boundary/horn sieves, an algebraic path-groupoid horn consumer,
and categorical decalage with levelwise fixed-tip cone fibres. A whole
varying-`HFiber` displayed family remains a named prerequisite; because that
gate was not met, the curated coinductive `SST` facade is deferred. The plan
explicitly avoids importing dTT/Narya as a second foundation or reproducing
Herbelin's external frame/coherence machinery.
...
is the completed sixth-spiral book ledger. It extends the checked exposition from
...
layer. Its locally promoted result is the 374-page draft `0.7.0-dev` edition,
whose deterministic PDF SHA-256 is
`d15287f7a841dfd25ad441c7fc8ff804425659ae0c292c2d8ae726fb46c9204b`. The plan owns
...
release, visual QA, and local promotion. No article expansion or remote
