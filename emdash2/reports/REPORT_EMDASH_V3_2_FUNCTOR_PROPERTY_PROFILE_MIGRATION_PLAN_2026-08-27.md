...
| `FPP-DOC-6` | complete, awaiting checkpoint | Synchronized AGENTS, Foundations, status/SOP, canonical syntax, README, predecessor-plan supersession notices, examples, generated assertion catalog, and the book chapter/evidence source. `book:check` is green with 159/159 evidence claims and 2,916 math spans. The tracked release Markdown/PDF are deliberately not promoted in this non-release branch: the PDF skill's mandatory `container_tools/mark_artifact_operation_started.mjs` helper is absent, so no PDF render was attempted or claimed. |
| `FPP-CLOSE-7` | in progress | Focused sources/reviewers and the directly affected central diagnostics are green; the latter completed in 28.9 seconds. Exact warning and LHS audits, catalog, TOC, book source/evidence and diff hygiene pass. No active source/reviewer uses `ReadablePseudoFunctorProfile`, `StrictFunctorData`, or `strict_functor_carrier`. Final documentation checkpoint and clean-worktree verification remain; long registered-source, examples, health and repository aggregates remain intentionally omitted because their unchanged boundaries do not block classification. |
