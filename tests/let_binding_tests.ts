...
describe("Let-Binding (Local Definition) Tests", () => {
    beforeEach(() => {
        ...
        const one = App(s, z);
        const two = App(s, one);
        defineGlobal("one", Nat, one, true);
        defineGlobal("two", Nat, two, true);
    ...
    it("should handle shadowing of a global variable", () => {
        ...
        defineGlobal("x_global", Def("Nat"), Def("two"), true);
    ...
    it("should handle nested let-bindings", () => {
        ...
        assert(areEqual(result.term, three, emptyCtx), "Nested let-bindings should evaluate correctly");
    });
