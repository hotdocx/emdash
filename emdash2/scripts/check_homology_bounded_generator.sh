...
reviewers=(
  ...
  examples/finite_arrow_tails.lp
  examples/finite_arrow_tail_init.lp
  examples/short_exact_row_zero.lp
  examples/homology_row_field_spans.lp
  examples/homology_bounded_generator.lp
  examples/homology_bounded_generator_arrows.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-bounded-generator-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-bounded-generator.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-bounded-generator.??????) ;;
...
printf 'homology-bounded-generator check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-bounded-generator checks succeeded; log: %s\n' "$log_file"
