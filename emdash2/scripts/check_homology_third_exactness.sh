...
reviewers=(
  examples/homology_third_exactness.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-third-exactness-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-third-exactness.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-third-exactness.??????) ;;
...
printf 'homology-third-exactness check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-third-exactness checks succeeded; log: %s\n' "$log_file"
