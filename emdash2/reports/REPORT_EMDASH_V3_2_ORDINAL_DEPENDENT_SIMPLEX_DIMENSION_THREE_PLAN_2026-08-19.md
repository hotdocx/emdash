...
`tmp/probes/ordinal_simplex3_visible_section_compositor.lp` validates the
stable owner
...
`section_total_func` is now promoted at the first position where
`piapp1_fapp0` is available.  The active kernel, focused generic owner probe,
focused checks, dimension-three compositor consumer, and strict LHS audit are
green; the audit reports zero unreviewed compound slots.  The warning inventory
is `1290 = 1131 + 159`, a classified `+11` unjoinable-critical-pair delta over
the prior `1279 = 1120 + 159` baseline.  Every new diagnostic is headed by the
arrow beta meeting a historical generic functor identity/composition or
naturality cut.  This is the expected current lax/strict prototype boundary:
the competing terms are related propositionally by the existing generic
functoriality paths, while the explicit Sigma arrow remains the intended
runtime projection owner.  No replaceable-pattern warning was added.

A single `make check` attempt was stopped under the standing no-long-aggregate
policy after the edited kernel and numerous unchanged downstream targets had
passed without a failure.  It is not used as completed evidence.  The direct
kernel target, focused active checks, generic consumer, warning inventory, and
strict audit are the promotion gates for this bounded tranche.  Wrong-section,
wrong-endpoint, noncollapse, retained-action, and final warning evidence remain
part of the source/closeout reviewer rather than a reason to rerun the long
aggregate now.

### 16.14 First-Hom Sigma Projection Boundary

The source continuation separated three issues that had previously appeared
as one endpoint mismatch.  The four diagnostic normal forms remain important:

```text
os3nsc_stable_source           |-> hom_postcomp_fapp0(...)
os3vsc_second_exchanged_source |-> Hom_fapp0(...)
os3nsc_stable_target           |-> hom_postcomp_fapp0(...)
os3fls_rigid_laxity_source     |-> Hom_fapp0(...)
```

They do not justify a new broad conversion from precomposition followed by
postcomposition to `Hom_fapp0`.  A rule-free generic probe now factors the
two-sided `Hom_fapp0(g,f,h)` action through the existing stable
`hom_precomp_along_fapp0` and `hom_postcomp_fapp0` midpoint, then validates the
actual dimension-three fold.  The residual difference is the concrete inner
section/internal-action arrow, not the ordinary Hom calculus.

The strict-prototype compositor endpoint comparison was also assembled
generically without identifying its nonidentity cell.  Equality action through
the decoded Sigma carrier then yields both its base equality and its dependent
fibre `PathOver`.  Separately, pulling an arbitrary displayed category back
along `Core_incl_func` proves that categorical transport along
`path_to_hom(p)` agrees with the established Path-category/J transport.  These
green probes establish the mathematical bridge; no simplex-specific equality,
normalizer, or Beck--Chevalley axiom is needed.

The remaining LF issue was first-hom projection.  `Sigma_proj1_func` already
computes on objects and constructor-visible total arrows, but a higher consumer
needs to expose the first component of an arrow in that hom.  Three candidate
interfaces were measured:

1. a broad rule on `fapp1_fapp0` was rejected because arbitrary higher arrows
   are only visibly Sigma-shaped after their endpoint arrows are
   constructor-visible;
2. staged rules on the hot `fapp0`/`fapp1_fapp0` heads typechecked in small
   probes but materially slowed the existing large dependency closure; and
3. a stable functor head avoided the global unifier cost, but its observation
   rules still paid the hot-eliminator matching cost.

The promoted prototype boundary therefore uses two transparent direct
operations in `emdash3_2.lp`:

```text
sigma_proj1_hom_fapp0(p)              := sigma_Fst(p)
sigma_proj1_hom_fapp1_fapp0(theta)     := sigma_Fst(theta).
```

The second signature deliberately carries constructor-visible endpoint
arrows.  This gives manifest subject reduction without introducing Sigma eta.
It also records the design fork: if later dimensions repeatedly require the
same explicit reframing, a cold primitive projection facade may replace these
transparent operations; the current result does not pre-empt that
consolidation.
...
wrapper overhead) before the new declarations ran.  Repeating that closure is
forbidden by the proportional-validation policy.

The next exact step is consequently bounded: reframe the already-checked
stable left and right total arrows to their explicit
`Struct_sigma(base,fibre)` presentations, apply
`sigma_proj1_hom_fapp1_fapp0` to the existing compositor, and consume the
result in the source-side whole comparison.  Validate it either in a smaller
standalone fixture or when the unchanged dependency again fits below the
required ceiling.  `ODS3-FACES-3` remains in progress; no native tetrahedron or
arbitrary-`H` completion claim is made by this projection tranche.

### 16.15 Source-Bridge Stop Condition And Architectural Reframe

The subsequent source-side probes have now reached the stop condition for
local endpoint reframing.  They establish the following positive facts:

1. the stable source compositor, the following section action, and the entire
   target-side chain are directed cells rather than requested endpoint
   equalities;
2. the displayed internal action admits the required opposite-oriented
   projection when its result is stated through the canonical
   `Obj(Hom_cat(...))` classifier; and
3. for the concrete equality-induced terminal reflag, that opposite projection
   supplies the required reverse-oriented component while retaining the same
   internal-action owner.

The second fact is **not** a claim that the compositor of an arbitrary lax
functor is invertible. It is an opposite/oplax observation of the existing
displayed action, usable here because the selected reflag is an
equality-induced strict-profile consumer. The analogous unrestricted
ordinary-functor reverse-compositor probe correctly does not typecheck.

After folding the opposite component through the represented Hom exchange,
its target is propositionally connected to the stable precomposition source.
Its source, however, is not definitionally the existing expected raw source:

```text
os3sped_reverse_source_folded
  !=def
os3nsc_expected_raw_source.
```

The typed `eq_refl` diagnostic fails in
`ordinal_simplex3_stable_precomp_endpoint_diagnostic.lp`; the retained log is
`ordinal_simplex3_stable_precomp_endpoint_diagnostic-20260821-021646.log`.
The expanded terms preserve two different construction histories. One is
assembled through the join/terminal section, its compositor, and represented
exchange. The other is projected through the native nested `homd_`/Sigma
presentation. Earlier `Hom_fapp0` versus `hom_postcomp_fapp0`, terminal eta,
Pi projection, Sigma first-hom projection, and variance issues were real
surface obstructions, but resolving them does not supply this whole
comparison.

This is therefore an API/design gap, not a mathematical obstruction and not a
license for another endpoint normalizer. The combinatorial ordinal-simplex
presentation and the native dependent-hom presentation have pointwise and
low-dimensional comparisons, but the active kernel does not yet expose their
comparison as one functorial higher owner. Dimension three is the first place
where the plan's retained-next-action requirement makes that omission
unavoidable.

Do not continue by adding a simplex-specific equality, a broad
`Hom_fapp0`/postcomposition conversion, Sigma eta, terminal eta, a primitive
`piapp*` head, or an assertion that the opposite component is a generic
inverse. Those routes have either been falsified by focused probes or would
erase directed information.

The next implementation tranche must instead formulate the smallest generic
whole **join/section-to-dependent-hom comparison**. Its acceptance boundary
is:

```text
whole join/section action
  -> whole native homd_/Sigma action,
```

with a typed object component, base-arrow action, the concrete dimension-three
component, one wrong-endpoint rejection, and one retained next action. First
attempt to define it transparently from the existing section-total,
represented-Hom, displayed-action, and Sigma projection owners. If that
composition has no canonical term, escalation item 3 permits one generic
propositional whole comparison at this semantic boundary; it must not be a
tetrahedron filler or a runtime endpoint collapse. If even that whole type
cannot be stated without reproducing the dimension-specific encodings, stop
this child plan and open a separately reviewed uniform decoder design of the
form

```text
decode_{n+1}(H)
  := dependent_join_step(decode_n(H restricted to Delta[n]),
                         new-vertex cross action of H).
```

The dimension-three construction would then be a consumer of that decoder,
not a second independent endpoint reconstruction. Until one of these two
whole designs is checked, `ODS3-FACES-3` remains in progress and the native
source, arbitrary-`H` map, public observation, and arbitrary-dimensional
claims remain unachieved.

### 16.16 Whole Section-Total/Postcomposition Comparison

The architecture gate in section 16.15 now has a checked generic owner. The
active kernel exposes

```text
section_total_mapped_func(FF,s)
  = Sigma(FF) o total(s)

section_total_postcomp_func(FF,s)
  = total(FF o s)

section_total_postcomp_transf(FF,s)
  : section_total_mapped_func(FF,s)
      ==> section_total_postcomp_func(FF,s).
```

Its point component computes to identity, its whole `tapp1_func` computes to
the mapped-total action, and its direct `tapp1_fapp0` joins that whole route.
The owner is placed after the existing `section_postcomp_sec` component and
action rules: an earlier placement correctly failed subject reduction because
those computations were not yet available to the rule checker.

The promoted owner has the following green evidence:

- the active `emdash3_2.lp` check;
- typed `eq_refl` checks for point, whole hom, and capped observations;
- a generic post/left laxity transformation with retained action;
- strict LHS audit with zero unreviewed compound slots; and
- unchanged warning inventory `1290 = 1131 + 159`.

The dimension-three consumer first applies this comparison to the
expected-to-visible reflag. Its post/left cell reframes through the existing
stable-postcomposition path, becomes constructor-visible, and exposes both
its Sigma first component and dependent next component. More decisively, the
consumer composes two instances:

```text
native section
  -- explicit normalizer --> expected section
  -- visible reflag      --> visible section.
```

Postcomposition of the first comparison by the second Sigma map initially
retains the stable `hom_postcomp_fapp0` functor endpoint. The existing typed
postcomposition path and a whole `Hom_func` reframe connect it to the raw
composite; no functor extensionality, rewrite, or unifier is added. Vertical
composition with the second comparison then yields one whole two-stage
transformation from the twice-mapped native total to the direct visible total.
Its action at the compositor base arrow has constructor-projectable source and
target totals, a base component, a dependent fibre component, and another
retained hom action.

This closes the former design gap. The remaining native packaging issue is a
propositional endpoint conjugation, not another missing whole owner. The
candidate target is the readable `p012`-composite fibre; it is intentionally
not definitionally the canonical transported `os3nna_qfibre`. The next exact
step is to reuse the existing `SigmaPathView` evidence
`os3qfp_fibre_categorical_path` (and the analogous represented-source path)
to reframe the new exchanged top cell to

```text
Hom(os3nna_ambient, os3nna_native_source, os3nna_qfibre).
```

The historical false-negative audit found that the source-bridge rejections
were already typed `eq_refl` diagnostics. A few early join diagnostics used
raw `assert ... ≡ ...`, but none owns this boundary. The new result therefore
does not depend on reinterpreting an untyped conversion failure as proof-time
equality.

### 16.17 Whole-Exchange-First Projection Correction

The post-checkpoint consumer audit validates the two-stage comparison but
corrects the proposed final order in section 16.16.  The following are now
green in focused ignored probes:

```text
two-stage whole transformation
  -> whole post/left laxity transformation
  -> constructor-visible Sigma/homd cell
  -> simultaneous base comparison and dependent higher component
  -> two directed endpoint factors with one common target.
```

This is positive evidence that the recursive construction contains both the
dimension-three factor and another retained action.  It is also negative
evidence for capped endpoint conjugation.  After the Sigma cell is projected
and its base component is separately mapped through represented covariance
and the native exchange, neither endpoint is judgmentally the older stable,
native, expected-raw, or equality-transported readable presentation.  The
same failures persist with typed `eq_refl`; they are not conversion-query
false negatives.  Likewise, the first comparison's point component is
identity before whole represented-Hom reframing, but evaluation after that
reframe is not judgmentally the independently assembled path-induced point
arrow.  The internal action deliberately retains the distinction.

The root cause is projection order, not a missing endpoint normalizer.  A
Sigma/homd cell constructs its base and dependent fibre together.  Mapping
only the projected base through the native exchange caps the construction too
early and loses the common owner which relates the two endpoint histories.
...
retained action remain the input to the corrected whole-exchange-first route.
It also does not yet complete `ODS3-FACES-3`, the native source tetrahedron, or
the arbitrary-`H` observation.

### 16.18 Doubly-Fibred Face Projections

The next focused audit confirms the user's geometric correction in the active
recursive classifier.  A dependent surface does not carry only the Sigma/base
projection.  Before all hom endpoints are capped, it is fibred over both its
base line and its target-fibre endpoint line.  In the selected fixed-endpoint
slice these two whole functors are
...
```text
fib_cov_tapp0_func(E,x,u,y)
  o dependent_triangle_base_face_func(E,x,u,y,v).
```

Consequently a volume `theta : surface0 -> surface1` has four whole face
readings:

```text
surface0
surface1
dependent_triangle_base_face_func[theta]
dependent_triangle_endpoint_face_func[theta].
```

The first two are its ordinary source and target.  The third is the dependent
hom's base surface.  The fourth is the endpoint-action surface.  Projecting
that fourth surface again shares the required lower line with the
corresponding projection of the base surface because both routes remain under
...
endpoint of the existing two-stage whole owner, then match its ordinary
source/target and projected base/endpoint surfaces with 012, 013, 023, and 123
through the already-existing join-cross comparisons.  Only after those four
whole comparisons are green should the cell be re-presented at
`DependentSimplex3_cat`; the post-laxity cell between the two endpoint
tetrahedra remains the separately retained dimension-four action.

### 16.19 The Doubly-Fibred Category of Surfaces

The preceding fixed-endpoint projection is valid, but it is one recursive
level below the most direct naming of the tetrahedron's four faces.  The
canonical native presentation instead starts with the category

```text
S2(C;x0,e01) = DependentSimplex2_cat(C,x0,e01)
```

whose objects are surfaces `01i`.  This category is doubly fibred over two
categories of lines.  The existing generic `FaceCode` action supplies the two
whole functors:

```text
dependent_simplex2_target_line_func : S2(C;x0,e01) -> S1(C;x0)
dependent_simplex2_base_line_func   : S2(C;x0,e01) -> S1(C;x1)
```

where the second codomain is retained intrinsically as the decoded `12`-face
code and computes to `S1(C;x1)` when `e01=(x1,p01)` is constructor-visible.
The product pairing

```text
dependent_simplex2_boundary_line_func
  : S2(C;x0,e01) -> S1(C;x0) x S1(C;x1)
```

is the required one-owner double fibration.  If

```text
theta : Hom_{S2(C;x0,e01)}(surface012,surface013),
```

then its four canonical surface readings are

```text
source(theta)                                      = surface012
target(theta)                                      = surface013
dependent_simplex2_target_line_func[theta]         = face023
dependent_simplex2_base_line_func[theta]           = face123.
```

This is the precise form of the user's geometric assignment.  The last two
equalities mean equality of the intrinsic whole face owners.  In today's lax
prototype, comparison with a separately rebuilt constructor such as `q023`
may retain an `fdapp1_int_hom_fapp0` identity-action cell rather than collapse
judgmentally; that is profile evidence, not a missing face and not permission
for a simplex-specific rewrite.

The three functors above are promoted transparently in
`emdash3_2_dependent_simplex_faces.lp`.  The focused reviewer checks, by typed
`eq_refl`, their constructor-visible decoded codomains, paired object action,
and both projections of the paired hom action.  The ignored probe
`tmp/probes/dependent_simplex3_doubly_fibred_faces.lp` additionally checks the
two hom actions on an arbitrary constructor-visible tetrahedral input.  Thus
the generic mechanism is not dimension-three source text generation: it is
the existing recursive `dependent_simplex_face_func`, valid for arbitrary
face codes and retaining another `fapp1_func` action at every level.

The ordinal task is now sharply stated.  Construct the one ordinal
`theta : surface012 -> surface013` from the retained join-cross internal
action, then obtain `023` and `123` by the two functor actions above.  Do not
continue trying to identify four named faces by applying the lower-level
fixed-endpoint functors to an already-capped composite boundary.  That lower
level remains useful for constructing `theta` and its next action, but the
surface-category double fibration is the canonical public observation.

Focused validation is green for the owning source, its reviewer, typed
product projections, visible decoded codomains, source TOC, active-reference
and report-header lints, catalog strictness, diff hygiene, and the strict LHS
audit (`0` unreviewed candidates).  A resumable `make health` refresh was
stopped after it began rechecking unchanged long algebraic-geometry targets;
it had already checked the dependent-simplex source chain successfully but
