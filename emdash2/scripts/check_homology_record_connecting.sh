...
reviewers=(
  examples/homology_record_connecting.lp
  examples/homology_record_connecting_whole.lp
  examples/homology_connecting_cover_whole.lp
  examples/homology_record_boundary_factors.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-record-connecting-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-record-connecting.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-record-connecting.??????) ;;
...
printf 'homology-record-connecting check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-record-connecting checks succeeded; log: %s\n' "$log_file"
