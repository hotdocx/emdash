...
| `CASC-COMP-2` | pending | Implement identity and composition for constructor-visible squares using existing whiskering/action/associativity owners. Record every reliance on the historical strict endpoint cuts; add no cubical-specific associativity equation. |
| `CASC-FACES-3` | in progress | Whole source and target functors plus constructor-visible object and square-action betas are promoted with `CASC-ARROW-1`. The continuation must add explicit retained next-hom evidence and validate the projection through the first iterated arrow level. |
| `CASC-MAP-4` | pending | Add `CubicalArrow_func(F)`, edge and square computations, source/target comparisons, identity/composition behavior, and retained next action. |
