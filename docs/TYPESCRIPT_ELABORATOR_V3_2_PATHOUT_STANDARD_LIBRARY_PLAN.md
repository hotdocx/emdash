...
separately approved at review checkpoint `397472f`, but measured compilation
...
The reviewed v14 root-only implementation is final-proportional-green at
semantic checkpoint `b6005b3`. It compiles all four opaque declarations,
...
implementation was attempted, but its authorization is superseded by the
measured counterevidence below.

### `PATHIND-TRUSTED-PROFILE-1C` corrected proposal v5

The v4 weak-head fusion subject-checks, but a cold semantic compile still
stops while inferring `pathind.fixed-source.fib-cov-section-point` with
`INVALID_RUNTIME_RULE_TYPE`: the residual heads are `object-classifier` and
`transfor-classifier`. This is a distinct presentation boundary. The generic
Core signature of `tapp0_fapp0` asks for `Transf(K,Cat_cat,E,D)`, whereas v4's
active reduction path reaches `Obj(Transf_cat(K,Cat_cat,E,D))`.

Active source lines 9150-9151 define exactly

```text
Transf(K,Cat_cat,E,D)
  ↪ Obj(Transf_cat(K,Cat_cat,E,D)).
```

That transparent-definition rule already has a subject-checked transfer in
the sibling fibred-product profile, but the selected PathOut predecessor
imports only its declaration linkage, not that runtime fragment. Importing
the whole sibling runtime profile would unnecessarily widen this slice, and
reversing the equation would contradict active reduction. The corrected
immutable proposal is
[`src/v3_2/pathind_fixed_source_proposal_v5.ts`](../src/v3_2/pathind_fixed_source_proposal_v5.ts),
with focused checks in
[`tests/v3_2_pathind_fixed_source_proposal_v5_tests.ts`](../tests/v3_2_pathind_fixed_source_proposal_v5_tests.ts).
It preserves v4 and adds only the active transparent `Transf` delta before
the six FibCov/PathInd rules.

The exact local boundary becomes **5/10/0/6**. Five runtime observations,
nine bounded-oracle assertions, the rho-section consumer, and eight strict
negatives remain unchanged. The correction adds no declaration owner,
mathematics, reversed reduction, runtime engine, Core/checker/evaluator
branch, active Lambdapi edit, or later/public effect. V4 checkpoints
`f4101e2` and `397472f` remain backtracking evidence but no longer authorize
implementation.

Corrected-v5 proposal validation on 2026-08-10 is:
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_fixed_source_proposal_v5.ts \
  tests/v3_2_pathind_fixed_source_proposal_v5_tests.ts \
  tests/main_tests.ts
  passed
...
No Lambdapi or long repository aggregate is relevant to this immutable-data
correction. Proposal v5 is non-authorizing, and checkpoint `7219828` freezes
those exact bytes.

The separate immutable review is
[`src/v3_2/pathind_fixed_source_review_v5.ts`](../src/v3_2/pathind_fixed_source_review_v5.ts),
with focused checks in
[`tests/v3_2_pathind_fixed_source_review_v5_tests.ts`](../tests/v3_2_pathind_fixed_source_review_v5_tests.ts).
It approves only checkpoint `7219828` under the user's standing unattended
delegation, with later human supersession. Authorization is limited to the
exact root-only **5/10/0/6** implementation: v4's closure plus the forward
active transparent `Transf` delta from lines 9150-9151. It expressly denies
reversing that delta, importing the whole sibling runtime profile, a generic
engine/checker/signature/body/owner change, every 1D/1E/1F or public effect,
active Lambdapi edits, and integration or release.

Corrected-v5 separate-review validation on 2026-08-10 is:
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_fixed_source_review_v5.ts \
  tests/v3_2_pathind_fixed_source_review_v5_tests.ts \
  tests/main_tests.ts
  passed
...
### `PATHIND-TRUSTED-PROFILE-1C` corrected proposal v6

A temporary environment-gated diagnostic, removed immediately after use,
captured the exact unreduced classifier pair at the third FibCov projection:
...
Thus v4's displayed-hom fusion and v5's `Transf` delta both subject-check but
cannot fire soon enough. Conversion intentionally does not unfold named
declaration bodies; once the inner `FibCov_target_catd` remains under
`fapp0`, the outer `fapp0` and `Transf` owners already differ.

The corrected immutable proposal is
[`src/v3_2/pathind_fixed_source_proposal_v6.ts`](../src/v3_2/pathind_fixed_source_proposal_v6.ts),
with focused checks in
[`tests/v3_2_pathind_fixed_source_proposal_v6_tests.ts`](../tests/v3_2_pathind_fixed_source_proposal_v6_tests.ts).
It preserves v5 and adds one subject-checked fusion of the complete forward
active path:
...
The derivation is pinned to active displayed-hom line 5481, `hom_con` object
line 7865, `hom_int` object line 8419, displayed-to-ordinary line 9177,
representable definitions at lines 13765-13775, and FibCov target definition
lines 13923-13928. The right side remains in the forward normal form consumed
by v5's `Transf → Obj(Transf_cat)` delta; no source equation is reversed.

The exact local boundary becomes **5/11/0/6**. Five runtime observations,
nine bounded-oracle assertions, the rho-section consumer, and eight strict
negatives remain unchanged. The fusion adds no mathematics, declaration-
unfolding engine, Core/checker/evaluator branch, signature/body/owner change,
active Lambdapi edit, or later/public effect. V5 checkpoints `7219828` and
`3f95e7c` remain backtracking evidence but no longer authorize implementation.

Corrected-v6 proposal validation on 2026-08-10 is:
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_fixed_source_proposal_v6.ts \
  tests/v3_2_pathind_fixed_source_proposal_v6_tests.ts \
  tests/main_tests.ts
  passed
...
No Lambdapi or long repository aggregate is relevant to this immutable-data
correction. Proposal v6 is non-authorizing, and checkpoint `b41c3b0` freezes
those exact bytes.

The separate immutable review is
[`src/v3_2/pathind_fixed_source_review_v6.ts`](../src/v3_2/pathind_fixed_source_review_v6.ts),
with focused checks in
[`tests/v3_2_pathind_fixed_source_review_v6_tests.ts`](../tests/v3_2_pathind_fixed_source_review_v6_tests.ts).
It approves only checkpoint `b41c3b0` under the user's standing unattended
delegation, with later human supersession. Authorization is limited to the
exact root-only **5/11/0/6** implementation: v5's closure plus the one exact
forward FibCov-target section fusion. It expressly denies declaration
unfolding, retaining the temporary checker diagnostic, reversing `Transf`, a
generic engine/checker/signature/body/owner change, every 1D/1E/1F or public
effect, active Lambdapi edits, and integration or release.

Corrected-v6 separate-review validation on 2026-08-10 is:
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_fixed_source_review_v6.ts \
  tests/v3_2_pathind_fixed_source_review_v6_tests.ts \
  tests/main_tests.ts
  passed
...
Semantic checkpoint `a361dc3` freezes those exact implementation, test, and
completion-ledger bytes.
...
#### Proposal checkpoint and separate review

Checkpoint `188b8e5` freezes the exact proposal bytes; the proposal module's
SHA-256 is
`da30d4fc2a9d54737e8fce9b0256e9b066b6b4f463d054d0d38741cdaedddd63`.
No immediate human objection followed that checkpoint. Under the user's
standing unattended delegation, the separate immutable review in
[`src/v3_2/pathind_internalized_review.ts`](../src/v3_2/pathind_internalized_review.ts)
approves decision `D-TS-EMDASH-PATHIND-INTERNALIZED-001` exactly as proposed,
with later human supersession. Focused review tests are in
[`tests/v3_2_pathind_internalized_review_tests.ts`](../tests/v3_2/pathind_internalized_review_tests.ts).

The review authorizes only root-local implementation of four opaque owners,
four runtime projections, zero proof rules, and ten transparent definitions
through the generic transfer engines. It requires `PathInd_transfd` to remain
the primary internally natural theorem, `PathInd_funcd` to remain a derived
transparent Sigma-total presentation, and both source-arrow and target higher
action to retain their internal owners. It explicitly denies importing the
whole scale-stress profile, adding an external naturality square, claiming
arbitrary non-cartesian Sigma naturality, changing a generic engine or active
Lambdapi source, entering any public/browser/package barrel, implementing
transitivity, or performing integration/release work.

Separate-review proportional evidence is:
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_internalized_review.ts \
...
`D-TS-EMDASH-PATHIND-INTERNALIZED-003` exactly as proposed. V3 is frozen at
checkpoint `5a1d635`, with SHA-256
`4c9b60411a7b1c98b3da44fdd6919360a3cf65a18e862c163d5f911a214308e3`.

The separate immutable review candidate is
[`src/v3_2/pathind_internalized_review_v3.ts`](../src/v3_2/pathind_internalized_review_v3.ts),
with focused tests in
[`tests/v3_2_pathind_internalized_review_v3_tests.ts`](../tests/v3_2/pathind_internalized_review_v3_tests.ts).
It authorizes only the exact one-for-one post-prefix replacement under the
user's standing unattended delegation, with later human supersession. It
explicitly denies retention of the v2 pre-prefix rule, any sixth runtime rule,
proof integration, generic runtime/checker changes, and every existing later
or public effect.
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_internalized_proposal_v3.ts \
  src/v3_2/pathind_internalized_review_v3.ts \
  tests/v3_2_pathind_internalized_proposal_v3_tests.ts \
  tests/v3_2_pathind_internalized_review_v3_tests.ts \
  src/v3_2/pathind_internalized_transfer.ts
  passed
...
`D-TS-EMDASH-PATHIND-INTERNALIZED-004` exactly as proposed. V4 is frozen at
...
It authorizes exactly four owners, four mathematical projections, the two
non-mathematical support fusions, zero proof rules, and ten definitions. It
denies a seventh runtime rule, generic category collapse, proof integration,
generic runtime/checker widening, and every later/public effect.
...
./scripts/pnpmw exec eslint \
  src/v3_2/pathind_internalized_proposal_v4.ts \
  src/v3_2/pathind_internalized_review_v4.ts \
  tests/v3_2_pathind_internalized_proposal_v4_tests.ts \
  tests/v3_2_pathind_internalized_review_v4_tests.ts \
  src/v3_2/pathind_internalized_transfer.ts
  passed
...
Semantic checkpoint `b6005b3` implements exact **4/13/0/10**: four opaque
...
### Corrected transitivity proposal v4 after reviewed-v3 replay

Cold execution of the reviewed v3 boundary preserves the useful positive
evidence: all five transparent definitions compile; its one local rule
subject-checks and fires at the exact instantiated rule redex; both typed
consumers, all eight negatives, capability sealing, and non-export closure
pass. Passing the fixture's descendant environment also lets inherited
provider `stress.sigma-pi.uncurrying` solve the intended proof-time category
presentation without adding a local proof rule.

The full focused replay nevertheless remains
`9-tests-6-pass-2-fail-1-skip`. The first failure now occurs only after that
inherited proof succeeds: a test helper asks the transitivity-local linkage
for predecessor symbol `path_ind_sec`. The correction is to use the canonical
fixed-source Core name. It changes no semantic boundary.

The second failure is the remaining semantic counterexample. An isolated
replay of the final-four-observations test reports one failure after about 194
seconds. Its normalized left side is the expected complete
`functor-object` presentation, but every `Rep_catd` subterm has already
delta-expanded to its stable `hom_(...,id,...)` body. The v3 pattern retains
`Rep_catd`, so it remains syntactically unreachable even though it is
well-typed and fires when instantiated directly. Importing more deltas or
adding a second support would enlarge the boundary without addressing the
ordering principle.

Corrected v4 therefore replaces v3 one-for-one with the original consumer
parent:

```text
fapp0(
  Hom_cat(Z,x,y), Functord_cat(Z,Rep_y,Rep_x),
  component(
    Z, Rep_x, CompTarget_catd(Z,x), y, path_comp_sec(Z,x)),
  p)
  ↦ path_comp_func(Z,x,y,p).
```

This exact root is present before any descendant delta is consulted. It is a
derived presentation fusion for one named consumer, not a new mathematical
equation. The boundary remains exactly **0/1/0/5**: zero trusted owners, one
derived runtime support, zero local proof rules, and five transparent
definitions. V2 and v3 are both replaced rather than retained.

The corrected, non-authorizing proposal is
[`src/v3_2/pathout_transitivity_proposal_v4.ts`](../src/v3_2/pathout_transitivity_proposal_v4.ts),
with focused tests in
[`tests/v3_2_pathout_transitivity_proposal_v4_tests.ts`](../tests/v3_2_pathout_transitivity_proposal_v4_tests.ts).
It pins v3 proposal/review checkpoints `fe1a9b7`/`0834d00`, their exact
digests, and ledger checkpoint `5d0dad5`. It preserves the definitions,
providers, consumers, seven-runtime/one-proof-time observation partition,
negatives, bounded oracle, and all generic/public/external denials. It
specifically denies retaining v3, adding a second rule, importing whole
`Rep_catd`, `hom_con`, or `id_funcd` behavior, or changing conversion.

Corrected-v4 proposal evidence on 2026-08-11 is:
...
node --require ts-node/register --test \
  ...
  47 tests / 7 suites: 47 passed, 0 failed

./scripts/pnpmw run typecheck
  passed

./scripts/pnpmw exec eslint \
  src/v3_2/pathout_transitivity_proposal_v4.ts \
  tests/v3_2_pathout_transitivity_proposal_v4_tests.ts
  passed
```
