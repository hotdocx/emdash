...
// --- End Report ---
let nextVarId = 0;
const freshVarName = (hint = 'v') => `${hint}${nextVarId++}`;
let nextHoleId = 0;
const freshHoleName = () => `?h${nextHoleId++}`;
const Type = () => ({ tag: 'Type' });
const Var = (name) => ({ tag: 'Var', name });
const Lam = (paramName, body, paramType) => ({ tag: 'Lam', paramName, paramType, body, _isAnnotated: !!paramType });
const App = (func, arg) => ({ tag: 'App', func, arg });
const Pi = (paramName, paramType, bodyType) => ({ tag: 'Pi', paramName, paramType, bodyType });
const Hole = (id) => ({ tag: 'Hole', id: id || freshHoleName(), ref: undefined });
const emptyCtx = [];
const extendCtx = (ctx, name, type) => [{ name, type }, ...ctx];
const lookupCtx = (ctx, name) => ctx.find(b => b.name === name);
const constraints = []; // Global store for constraints
function addConstraint(t1, t2, origin) {
    constraints.push({ t1, t2, origin });
}
function getTermRef(term) {
    if (term.tag === 'Hole' && term.ref)
        return getTermRef(term.ref);
    return term;
}
function whnf(term, ctx, depth = 0) {
    if (depth > 100)
        throw new Error("WHNF depth exceeded");
...
function normalize(term, ctx, depth = 0) {
    if (depth > 100)
        throw new Error("Normalize depth exceeded");
    const current = getTermRef(term);
    switch (current.tag) {
        case 'Type':
        case 'Var':
        case 'Hole': return current;
        case 'Lam':
...
function areEqual(t1, t2, ctx, depth = 0) {
    if (depth > 50)
        throw new Error("Equality check depth exceeded");
    ...
    if (normT1.tag !== normT2.tag)
        return false;
    switch (normT1.tag) {
        case 'Type': return true;
        case 'Var': return normT1.name === normT2.name;
        case 'App':
            return areEqual(normT1.func, normT2.func, ctx, depth + 1) &&
                areEqual(normT1.arg, normT2.arg, ctx, depth + 1);
        case 'Lam': {
            const t2Lam = normT2;
...
function termContainsHole(term, holeId, visited) {
    const current = getTermRef(term); // Follow existing refs
    if (current.tag === 'Hole') {
        if (current.id === holeId)
            return true;
    ...
    if (visited.has(printTerm(current)))
        return false; // Avoid cycles in non-HOAS term graph
...
function unify(t1, t2, ctx, depth = 0) {
    if (depth > 50)
        throw new Error(`Unification depth exceeded: ${printTerm(t1)} vs ${printTerm(t2)}`);
    const normT1 = getTermRef(t1); // Chase refs
    const normT2 = getTermRef(t2);
    if (normT1 === normT2 && normT1.tag !== 'Hole')
        return true; // Physical identity (and not an unsolved hole)
    if (areEqual(normT1, normT2, ctx))
        return true; // Already definitionally equal
    // Handle hole cases (symmetric)
    if (normT1.tag === 'Hole')
        return unifyHole(normT1, normT2, ctx, depth);
    if (normT2.tag === 'Hole')
        return unifyHole(normT2, normT1, ctx, depth);
    // Different tags, not holes -> fail
    if (normT1.tag !== normT2.tag)
        return false;
    switch (normT1.tag) {
        case 'App':
            return unify(normT1.func, normT2.func, ctx, depth + 1) &&
                unify(normT1.arg, normT2.arg, ctx, depth + 1);
        case 'Lam': {
            const t2Lam = normT2;
            ...
            if (normT1.paramType && t2Lam.paramType) {
                if (!unify(normT1.paramType, t2Lam.paramType, ctx, depth + 1))
                    return false;
            }
...
function unifyHole(hole, term, ctx, depth) {
...
function infer(ctx, term, depth = 0) {
    if (depth > 50)
        throw new Error(`Type inference depth exceeded for ${printTerm(term)}`);
    const current = getTermRef(term);
    switch (current.tag) {
        ...
        case 'Var':
            ...
            if (!binding)
                throw new Error(`Unbound variable: ${current.name}`);
            return binding.type;
        case 'Hole':
...
function check(ctx, term, expectedType, depth = 0) {
    if (depth > 50)
        throw new Error(`Type checking depth exceeded for ${printTerm(term)} : ${printTerm(expectedType)}`);
    const current = getTermRef(term);
...
function elaborate(term, expectedType, initialCtx = emptyCtx) {
    constraints.length = 0;
    nextVarId = 0;
    nextHoleId = 0; // Reset gensym counters
    let finalTypeToReport;
    if (expectedType) {
...
function printTerm(term, boundVars = []) {
...
try {
    ...
    if (!areEqual(result.type, expected, baseCtx))
        throw new Error("Type mismatch for ex1");
    console.log("   Correct.");
}
catch (e) {
    console.error("Error in example 1:", e.message);
}
...
try {
    ...
    if (!areEqual(result.type, targetType, baseCtx))
        throw new Error("Type mismatch for ex2 type");
    ...
    console.error("Error in example 2:", e.message);
}
...
try {
    ...
    if (result.type.tag === 'Pi' && result.type.paramType.tag === 'Hole') {
        ...
        if (bodyYieldsParamHole) {
            ...
            throw new Error("Body type of Pi does not match param hole for ex3");
    }
    else
        throw new Error("Inferred type for unannotated id not Pi with hole for ex3");
}
catch (e) {
    console.error("Error in example 3:", e.message);
}
...
try {
    ...
    if (!areEqual(holeTypeResult.type, Nat, baseCtx))
        throw new Error("Hole type not Nat for ex4");
    console.log("   Correct.");
}
catch (e) {
    console.error("Error in example 4:", e.message);
}
    ...
    if (result.type.tag === 'Pi') { // Outer Pi for f
        ...
        if (typeOfF.tag === 'Pi' && typeOfF.paramType.tag === 'Hole' &&
            ...
            if (typeOfF_body.tag === 'Hole' && bodyTypeOuter_body.tag === 'Hole' &&
                ...
                throw new Error("?B holes do not match for ex5");
        }
        else
            throw new Error("?A holes do not match or structure incorrect for ex5");
    }
    else
        throw new Error("Overall type not Pi for ex5");
}
catch (e) {
    console.error("Error in example 5:", e.message);
}
...
try {
    ...
    console.log(`\n6. Check (λx:Nat. x) against Bool: Caught expected error: ${e.message.slice(0, 100)}...`);
}
...
try {
    ...
    if (!areEqual(result.type, Bool, baseCtx))
        throw new Error("App result type not Bool for ex7");
    // Check type of fHole
    ...
    if (!areEqual(fHoleTypeResult.type, expectedFHoleType, baseCtx))
        throw new Error("Hole type for f not Nat->Nat for ex7");
    console.log("   Correct.");
}
catch (e) {
    console.error("Error in example 7:", e.message);
}
...
try {
    ...
    console.error("Error in example 8:", e.message);
}
...
try {
    ...
    if (!areEqual(result.type, concretePiType, baseCtx))
        throw new Error("Type mismatch for ex9");
    console.log("   Correct.");
}
catch (e) {
    console.error("Error in example 9:", e.message);
}
