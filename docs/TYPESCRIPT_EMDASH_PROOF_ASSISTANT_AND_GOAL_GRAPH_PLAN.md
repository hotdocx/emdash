...
| `AGENT-EVAL-12B4` | Reproducible real-agent runs and measured graduation | one R9 minimized-receipt digest authorized after checkpoint | Exact proposal `c1145d7`, coreutils binary, path, closed environment, and one-way output pass review. Only one receipt hash command is authorized; stderr remains untouched. |
