...
function fixedPublicationDate(manifest) {
  return new Date(manifest.publicationDate + 'T00:00:00.000Z');
}

function canonicalizeStructureNodeIds(pdf) {
  // Chromium assigns process-local IDs such as node00019354 to tagged table
  // headers. The IDs and their /Headers references are semantically opaque,
  // but a cold render can shift every suffix by one. Rewrite only parsed PDF
  // string objects so content and font streams remain byte-for-byte opaque.
  ...
  if (ordered.length > 99999999) {
    throw new Error('Tagged PDF contains too many structure node IDs to canonicalize');
  }
...
function normalizePdfMetadata(rawPath, normalizedPath, manifest) {
  return (async () => {
    const pdf = await PDFDocument.load(fs.readFileSync(rawPath), {
      updateMetadata: false,
    });
    const publicationDate = fixedPublicationDate(manifest);
    const authors = manifest.authors.join(', ');

    // Chromium inserts time-dependent XMP metadata. The Info dictionary below
    // is the selected deterministic metadata owner for this release artifact.
    pdf.catalog.delete(PDFName.of('Metadata'));
    pdf.setTitle(manifest.displayTitle);
...
async function main() {
  ...
  const temporaryDirectory = path.join(REPO_ROOT, 'tmp', 'pdfs');
  const stem = manifest.id + '-' + process.pid;
  ...
  const onSignal = (signal, exitCode) => {
    void cleanup().finally(() => {
      console.error('Book PDF export interrupted by ' + signal);
  ...
  try {
    ...
    page.on('console', (message) => {
      ...
      if (message.type() === 'warning' &&
          (text.includes('LaTeX-incompatible input') || text.includes('[mathVsTextAccents]'))) {
    ...
    if (failures.length > 0) {
      throw new Error('PDF browser gate failed:\n- ' + failures.join('\n- '));
    }
    ...
    await normalizePdfMetadata(rawPath, normalizedPath, manifest);
    execFileSync(
      'qpdf',
      [
        '--deterministic-id',
    ...
    console.log(
      'book PDF exported: ' + path.relative(REPO_ROOT, outputPath) +
      ', pages=' + pageCount + ', sha256=' + sha256(outputPath)
    );
  } finally {
...
main().catch((error) => {
  console.error('book PDF export failed: ' + (error?.message || error));
  process.exitCode = 1;
});
