...
describe("Dependent Types Tests: Length-Indexed Vectors (Vec)", () => {
    ...
    it("should elaborate vnil for a specific type", () => {
        ...
        assert(areEqual(result.type, expectedType, emptyCtx), `vnil {Bool} should have type Vec Bool z. Got ${printTerm(result.type)}`);
    });
    ...
    it("should elaborate a Vec of length 1", () => {
        ...
        assert(areEqual(result.type, expectedType, emptyCtx), `Vec of length 1 has wrong type. Got ${printTerm(result.type)}`);
    });
    ...
    describe("Vector Append Function", () => {
        ...
        it("should correctly type-check an application of append", () => {
            ...
            assert(areEqual(result.type, expectedType, emptyCtx), `Append type check failed. Got ${printTerm(result.type)}`);
        });
    });
}); 