...
export function areStructurallyEqualNoWhnf(t1: Term, t2: Term, ctx: Context, depth = 0): boolean {
    ...
    switch (rt1.tag) {
        ...
        case 'Let': {
            ...
            let typeOk = true;
            if (rt1._isAnnotated) {
                if (!rt1.letType || !let2.letType || !areStructurallyEqualNoWhnf(rt1.letType, let2.letType, ctx, depth + 1)) {
                    typeOk = false;
                }
            }
            if (!typeOk) return false;

            if (!areStructurallyEqualNoWhnf(rt1.letDef, let2.letDef, ctx, depth + 1)) return false;

            const freshVName = rt1.letName;
            const freshV = Var(freshVName, true);
            const letDefTypeForCtx = (rt1._isAnnotated && rt1.letType) ? getTermRef(rt1.letType) : Hole(freshHoleName()+"_structEq_unannot_let_param");
            const extendedCtx = extendCtx(ctx, freshVName, letDefTypeForCtx, Icit.Expl);
            return areStructurallyEqualNoWhnf(rt1.body(freshV), let2.body(freshV), extendedCtx, depth + 1);
        }
        case 'ObjTerm':
