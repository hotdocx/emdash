...
while `eq_refl` itself remains eliminator-visible for Sigma and generic
reflexivity. The first Pi/function path-view slice is promoted as well:
`PiPathView`, fully-applied reflexive Pi observation through `eq_apd`,
`pi_path_refl` as a compatibility alias for `eq_refl`, identity/coercion
`pi_path_encode`, and direct Pi equality computation. The generic reflexive
`ind_eq` rule at Pi remains active because `eq_refl(Pi_grpd(A,B),f)` itself is
not rewritten; only its full related-input application is observed.
`pi_path_apply` is deliberately not a stable owner in the current design. The
first
...
regression checks. Product groupoid element equality computes to the
constant-family Sigma path normal form. Universe-level Product paths are now
handled by a componentwise introduction head `product_grpd_path`, not by
decomposing arbitrary paths
`Product_grpd(A,B) = Product_grpd(A',B')`. The decoder
`grpd_equiv_path(product_type_equiv(eA,eB))` produces that introduced product
path, and `idtoequiv_grpd` maps introduced product paths back to
`product_type_equiv`. The `IsEquivMap` witnesses are intentionally isolated
...
The reflexive evidence constructors `type_equiv_refl` and
`iso_evidence_refl` are now stable primitive heads with projection rules for
their existing semantic components, so `grpd_equiv_path(type_equiv_refl)`,
`iso_evidence_path(iso_evidence_refl)`, and
`omega_equiv_path(omega_equiv_refl)` all compute to reflexivity. The first
constructor decoder is also promoted: `const_pathover` bridges ordinary fibre
paths over constant families, and
...
14. The operational decoder should also be reflected in the univalence
    ...
    an equivalent specified-inverse formulation whose forward and reverse maps
    are both operational, for example `idtoequiv_cat` and
    `omega_equiv_path`. This makes both intended runtime directions explicit
    while migrating the existing `idto*` names from induction-only definitions
    ...
    interfaces.
15. The immediate equality/reflexivity architecture is
    projection/application based. Keep `eq_refl` eliminator-visible at
    runtime for Sigma, Pi, and generic reflexivity; make `eq_refl` an
    injective head so focused constructor observers can compute. For Pi, the
    promoted rule is fully applied:

    ```text
    eq_refl(Pi_grpd(A,B),f)(x0,x1,p) -> eq_apd(f,x0,x1,p).
    ```

    A 2026-06-26 probe also showed that the partial first-application rule
    `eq_refl(Pi_grpd(A,B),f)(x0) -> eq_apd(f,x0)` and the fully-applied rule
    both make `eq_refl(Pi_grpd(A,B),f)` convertible to `eq_apd(f)` by
    Lambdapi eta, and both even prove the section-level comparison
    `eq_refl(Pi_grpd(A,B)) ≡ eq_apd(A,B)`. The fully-applied rule is promoted
    because it is the narrower and more conventional observer: it keeps
    partial applications of `eq_refl(f)` intact unless a concrete
    related-input path observation is requested. The partial `$x0` rule is
    recorded as feasible but not promoted. The generic reflexive `ind_eq`
    regression at Pi remains valid. For Sigma/Product, keep
    projection-consumer rules on `sigma_Fst` and `sigma_Snd`; do not rewrite
    Sigma `eq_refl` itself.
16. `eq_apd` is no longer intended to be primitive. A 2026-06-25/26 full-core
    ...
    This remains compatible with the fully-applied Pi `eq_refl` observer.
...
For Sigma, the owner is the Sigma constructor. Product initially inherited
this because `Product_grpd(A,B)` was a transparent constant-family Sigma:
...
After the 2026-06-25 Product groupoid migration, `Product_grpd(A,B)` is a
stable constructor with an explicit `τ` decoding and its own equality rule:

```text
τ(Product_grpd(A,B)) ↪ Σ x : A, B
...
typechecks, and Product element equality then computes through the transparent
Product-as-Sigma presentation. The current promoted form keeps the same
normal form but routes it through the stable `Product_grpd` rule documented
above.

Current status: Product element equality now computes through the stable
Product rule above, not through transparent unfolding.
...
Pi/function equality has a different implementation boundary. A separate
term-former `PiPathView(f,g) -> f = g` would be a functional-extensionality
principle. Under `UNI-EQ-COMPUTE`, the intended rule is instead:
...
This rule is now promoted. Functional extensionality is built into the
equality normal form for Pi types: a `PiPathView` term is already a term of
`f = g` by conversion. The design therefore does not require a separate
`pi_path_decode` owner. The remaining hard part is not the existence of
function extensionality, but transport/coherence over the resulting Pi path
normal form.

A focused 2026-06-25/26 review showed that the Sigma projection pattern
transfers to Pi cleanly as a fully-applied observer rule, provided `eq_refl`
is an injective rewrite head. The promoted reflexive Pi rule is:

```text
eq_refl(Pi_grpd(A,B), f)(x0,x1,p) ↪ eq_apd(f,x0,x1,p).
```

Lambdapi eta still proves the fixed-function and section-level comparisons
against `eq_apd`, but runtime normalization keeps `eq_refl(f)` itself visible.
Therefore the generic reflexive eliminator regression
`ind_eq(eq_refl_f, M, u) ↪ u` still passes. The one-argument observer
`eq_refl(f)(x0) -> eq_apd(f,x0)` was probed and is feasible, but is not
promoted because it introduces partial `eq_apd` normal forms before any
concrete path argument is observed.

The 2026-06-26 probes also showed that `pi_path_encode` should not be
induction-owned after direct Pi equality is active. Since
`@= (@Pi_grpd A B) f g` already computes to `PiPathView(A,B,f,g)`,
`pi_path_encode` is now the identity/coercion on its path argument. This keeps
`pi_path_encode(eq_refl f)` computing to `pi_path_refl(f)` without requiring
`pi_path_encode` to perform equality induction.

The earlier `pi_path_apply` stable-head design is retired. `pi_path_refl(f)`
remains a compatibility/readability alias for `eq_refl(f)`, so the
fully-applied Pi observer covers both spellings by unfolding and Lambdapi
function eta.
...
The same point applies to the forward direction. Keep the established
`idto*` names as the public forward maps:

```text
idtoequiv_grpd : A = B -> TypeEquiv A B
idtoiso_cat    : x = y -> IsoEvidence C x y
idtoequiv_cat  : x = y -> OmegaEquiv C x y
```

These names have now been migrated to stable operational forward heads while
preserving their reflexive behavior by rewrite rules. In the observational
equality design, later constructor-specific rules should attach to these same
heads rather than to separate throwaway aliases. For example, after a Product
object equality has a stable component path normal form, `idtoequiv_cat` for
that product path can construct `omega_equiv_product` from the component
forward maps. This remains the missing dual of the now-active Product
`omega_equiv_path` decoder.

The active `ua_grpd`, `isotoid_cat`, and `equivtoid_cat` maps can remain
semantic interfaces or compatibility wrappers, but they should not be the
primary runtime owners for constructor-specific reverse univalence
computation. In the final design, the preferred computational capability
should name the operational `idto*` forward maps and operational reverse maps
explicitly.

### Specified-Inverse Univalence Capabilities

The current `IsEquivMap` formulation is semantically correct but hides the
chosen inverse inside a contractible fibre proof, and the current
induction-defined implementations of the `idto*` maps only compute by
reflexivity. For computational univalence, the project needs an additional
specified-inverse formulation using operational `idto*` forward maps and
operational reverse maps.
...
where `f : A -> B` is the operational `idto*` path-to-equivalence map and
`g : B -> A` is the operational equivalence-to-path decoder. The actual
implementation can use `Product_grpd`/Sigma packaging, and the homotopies can
remain proof capabilities at first. Runtime computation is owned by `f` and
`g`, not by these proof fields.
...
This is now stated in Lambdapi using stable operational `idto*` heads and
stable reverse decoder heads. Their reflexive behavior is preserved by rules
such as `idtoequiv_cat(eq_refl x) ↪ omega_equiv_refl x`; constructor-specific
non-reflexive cases can be added incrementally at these same owners. Proving
or deriving the old contractible-fibre `IsEquivMap` formulation from the
specified-inverse formulation may require additional Sigma/path algebra, so
that derivation
...
`Product_grpd(A,B)` is now a stable constructor that decodes to a
constant-family Sigma at the object level. Product equality still uses the
Sigma equality normal form through an explicit Product rule:
...
ordinary component paths. It was not a prerequisite for the first direct
equality-computation slice, because Sigma equality can keep the `PathOver`
normal form directly. It has now been promoted because the first Product
categorical decoder consumes exactly this bridge:

```text
omega_equiv_path(omega_equiv_product(eA,eB))
  ↪ (omega_equiv_path(eA),
     const_pathover(omega_equiv_path(eA), omega_equiv_path(eB))).
```
...
However, the specified-inverse univalence laws give a concrete and useful
test bed for future transport computation. For example:
...
where `f` is an operational `idto*` map such as `idtoequiv_cat` or
`idtoequiv_grpd`, and `g` is an operational decoder such as
`omega_equiv_path` or `grpd_equiv_path`. In the Product/Sigma case, `g`
computes to a constructor path normal form with component equalities, and `f`
should be defined by constructor case analysis on that normal form rather than
by plain equality induction alone. The second
round-trip law:
...
is therefore a focused place where definitional computation can work for
constructor evidence such as `omega_equiv_product eA eB`: `g` produces the
pair/Sigma path and `f` maps it back componentwise to
`omega_equiv_product eA eB`. This gives a concrete probe for
constructor-specific coherence without requiring a general solution for
arbitrary `ind_eq` transport.

Operational `idto*` constructor rules should therefore be understood as
specialized equality eliminators for known equality normal forms. They are not
intended to replace `ind_eq`/`ind_eqr` for arbitrary motives. Rather, they give
the first concrete cases of the same principle: when the equality proof has a
constructor-shaped normal form and the motive is recognizable, computation can
follow the constructor. For example, if a future motive over a product object
is recognized as factoring through projections,

```text
P(xy) = Q(fst(xy), snd(xy)),
```

then transport along a product path should plausibly decompose into the
corresponding component transports. Once the direct operational `idto*`
constructor cases are stable, repeated patterns of this form can be factored
into shaped `ind_eq`/`ind_eqr` transport rules. That factoring is deliberately
later than the direct operational rules: it validates the univalence skeleton
first and avoids making full dependent transport computation a prerequisite
for constructor-specific univalence.
...
operational forward/reverse univalence maps compute by constructor cases;
...
internal path in Grpd
  -> TypeEquiv;                 // idtoequiv_grpd

internal object path in C
  -> directed arrow in C;

internal object path in C
  -> IsoEvidence(C,x,y);         // idtoiso_cat
...
Its promoted computation includes direct reflexive application, identity-style
encode, and direct equality classification:

```text
pi_path_refl(f) -> eq_refl(f);
eq_refl(Pi_grpd(A,B),f)(x0,x1,p) -> eq_apd(f,x0,x1,p);
eq_refl(Pi_grpd(A,B),f) == eq_apd(f);      // by eta/conversion
eq_refl(Pi_grpd(A,B)) == eq_apd(A,B);      // by eta/conversion
pi_path_refl(f)(x,x,refl_x) -> refl_(f x);
pi_path_encode(refl_f) -> pi_path_refl(f).  // by identity/coercion
@= (@Pi_grpd A B) f g -> PiPathView(A,B,f,g).
```

There is no standalone `PiPathView(f,g) -> f = g` decode, because
`PiPathView(A,B,f,g)` is now the normal form of `f = g`.
...
logs/probes/pi_rule_x0_probe-20260626-044533.log
logs/probes/pi_rule_full_app_probe-20260626-044533.log
logs/probes/pi_path_encode_identity_probe-20260626-043105.log
...
After the 2026-06-26 fully-applied Pi observer and identity-encode correction,
`make warning-summary` reports:
...
`pi_path_encode`, or the fully-applied Pi `eq_refl` observer.
...
encode/decode cancellation and direct Pi equality/coherence. Keeping these
...
identity/coercion `pi_path_encode`, fully-applied Pi `eq_refl` observation,
and direct Pi equality are active. The promoted equality rule is
`@= (@Pi_grpd A B) f g ↪ PiPathView(A,B,f,g)`, not a separate
`PiPathView -> f = g` decoder. The specialized reflexive observation
`(eq_refl f) x0 x1 p` computes to `eq_apd(f,x0,x1,p)`, while
`eq_refl(Pi_grpd(A,B),f)` itself stays eliminator-visible and only compares
to `eq_apd(f)` by eta.
...
names both maps whose constructor cases should compute. The existing
`idtoequiv_cat` name is kept as the public forward map. It is now a stable
operational head with a reflexivity rule; constructor-specific non-reflexive
rules should be added at this owner as their equality normal forms stabilize.
The
...
- constructor-specific forward/reverse computation has stable operational
  owners;
...
2026-06-25 decoder extension status: promoted. The operational reverse map now
computes on explicit Product omega-equivalence evidence:

```text
omega_equiv_path
  (Product_cat(A,B), (x0,y0), (x1,y1), omega_equiv_product(eA,eB))
  ↪
  Struct_sigma
...
```text
omega_equiv_product(omega_equiv_refl, omega_equiv_refl)
  ↪ omega_equiv_refl(Product_cat(_,_), _).
```

The two paths expose different intentional boundaries: decoder-first exposes
the Sigma/Product path normal form, while reflexive-collapse-first preserves
the generic reflexivity head. This is classified as a stable observational
equality boundary, not a blocker. Do not force a global rewrite of every
...
2026-06-25 groupoid Product univalence extension status: corrected and
promoted. The groupoid-level Product type former is now a stable constructor
rather than a transparent alias for Sigma:
...
The existing element equality behavior is preserved by a Product-specific
rule:
...
`product_type_equiv` is also now a stable head with projection rules for its
existing forward map and proof capability.

The first attempted universe-level rule was:
...
That rule is rejected as semantically overstrong for univalent `Grpd`.
Univalent equality between product type formers corresponds to arbitrary type
equivalence, and arbitrary equivalences between products do not split into
equivalences of the two chosen factors. For example, a swap equivalence or
other non-factor-preserving equivalence between products should not imply
`A = A'` and `B = B'`.

The corrected implementation keeps `ProductGrpdPathView` as the classifier of
componentwise product path data and introduces an explicit path constructor:

```text
ProductGrpdPathView(A,A',B,B')
  := Σ pA : A = A', PathOver(λ _ : Grpd, Grpd, pA, B, B');

product_grpd_path(pA,pB)
  : Product_grpd(A,B) = Product_grpd(A',B');

grpd_equiv_path(product_type_equiv(eA,eB))
  ↪ product_grpd_path(
      grpd_equiv_path(eA),
      const_pathover(grpd_equiv_path(eA), grpd_equiv_path(eB)));

idtoequiv_grpd(Product_grpd(A,B), Product_grpd(A',B'), product_grpd_path(pA,pB))
  ↪ product_type_equiv(
      idtoequiv_grpd(A,A',pA),
      idtoequiv_grpd(B,B',const_pathover_path(pA,pB))).
```

The active checks cover the `ProductGrpdPathView` classifier, the type of
`product_grpd_path`, the product decoder
`grpd_equiv_path(product_type_equiv(eA,eB))`, the forward `idtoequiv_grpd`
rule on introduced product paths, and the explicit reflexive introduced
product path case. This preserves the valid direction:
...
without asserting the invalid converse.
...
5. Direct Pi equality computation and fully-applied Pi `eq_refl` observation
   are now promoted under `UNI-EQ-COMPUTE`; the remaining Pi equality work is
   shaped eliminator/transport computation for non-reflexive constructor paths
   and coherence beyond the reflexive observer.
...
3. Add Pi/function path views only after `Pi_grpd`. The first Pi slice has
   ...
   encode. Direct Pi equality computation belongs to `UNI-EQ-COMPUTE` via
   `@= (@Pi_grpd A B) f g ↪ PiPathView(A,B,f,g)`.
...
1. Define explicit `CatUnivalence(C)` as equivalence of canonical
   `idtoequiv_cat`.
2. Migrate `idtoequiv_cat` to a stable operational forward head, preserving
   reflexivity computation and adding constructor-specific rules.
3. Define reverse heads such as `omega_equiv_path` as stable operational
   decoders.
4. Define `CatUnivalenceByDecoder(C)` as a specified-inverse capability for
   the operational heads.
5. Keep `equivtoid_cat` as a semantic compatibility wrapper and add only
   required coherence comparisons.
6. Add instances for `Path_cat`, `Op_cat`, and products.
7. Adopt global `cat_univalence(C)` and `cat_univalence_by_decoder(C)` as the
   intended operational foundation.
8. Record the immediate `Cat_cat` instance while leaving universe-specific
...
| `UNI-GRPD-PI` | promoted | none | further Pi equality/univalence begins | `Pi_grpd` formation and the related-input Pi path view are active; direct Pi equality computation is owned by `UNI-EQ-COMPUTE`. |
...
| `UNI-PI-VIEW` | related-input reflexive view, identity/coercion encode, fully-applied Pi `eq_refl` observer, and direct equality promoted | `UNI-PATHOVER`, `UNI-GRPD-PI` | Pi-constructor univalence or non-reflexive projection computation is consumed | Direct Pi equality is active as `@= (@Pi_grpd A B) f g ↪ PiPathView(A,B,f,g)`. Do not add a separate `PiPathView -> f = g` decoder. `(eq_refl(Pi_grpd(A,B),f)) x0 x1 p` computes to `eq_apd(f,x0,x1,p)`, and `pi_path_refl(f)` is a compatibility alias for `eq_refl(f)`. `pi_path_encode` is identity/coercion across the direct equality normal form. Generic reflexive `ind_eq` at Pi remains active because `eq_refl(f)` itself is not rewritten. `pi_path_apply` is retired. |
...
| `UNI-EQ-COMPUTE` | Sigma and Pi slices promoted 2026-06-25/26 | `UNI-SIGMA-VIEW`, `UNI-PI-VIEW`, `UNI-PATHOVER` | Product/Sigma/Pi decoder computation or more constructor paths are consumed | `=` and `eq_refl` are injective; Sigma equality reduces to `SigmaPathView`; Pi equality reduces to `PiPathView`; Sigma `eq_refl` remains eliminator-visible; `sigma_Fst`/`sigma_Snd` projection rules expose reflexive Sigma components; the targeted Sigma `eq_refl` unification bridge lets `sigma_path_refl` elaborate against generic reflexivity; fully-applied Pi `eq_refl` observation exposes `eq_apd` while preserving generic Pi reflexive elimination. |
...
| Product path views | stable Product element equality, direct Sigma/Product equality, and both constant-family bridges promoted | encode/decode orientation and projection overlap |
| observational Pi path | related-input slice, direct Pi equality, and reflexive apply projection promoted | higher substitution/transport coherence beyond reflexivity |
...
| Product groupoid univalence decoder and forward `idtoequiv_grpd` | corrected and promoted | stable Product head is a normal-form migration; arbitrary Product-universe paths are not decomposed; opaque/non-Product type constructors remain stuck |
...
warning-enabled deltas are classified rather than used as a veto.
```
...
ProductGrpdPathView classifies componentwise product path data;
product_grpd_path introduces componentwise Product-universe paths without
decomposing arbitrary Product-universe paths;
grpd_equiv_path(product_type_equiv(eA,eB)) produces product_grpd_path of the
component decoder paths;
idtoequiv_grpd maps introduced product_grpd_path terms to product_type_equiv
of the component idtoequiv_grpd images;
...
decoder heads;
type_equiv_refl and iso_evidence_refl are stable heads with projection rules;
grpd_equiv_path, iso_evidence_path, and omega_equiv_path compute on reflexive
evidence;
...
CatUnivalenceByDecoder(C) states that idtoequiv_cat and
omega_equiv_path are operational inverses;
