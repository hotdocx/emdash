...
the generic fibrewise dependent-Sigma extension is
`emdash3_2_fibrewise_sigma.lp`;
...
`emdash3_2_sites.lp`;
the whole ordinary-sieve extension and topology-local-object library is
`emdash3_2_sieve_extensions.lp`;
...
the supplied whole sheaf-restriction and sheaf-basis-equivalence extension is
...
the generic strict ordinary/displayed pointwise-to-whole fixed-forward
equivalence extension is `emdash3_2_strict_pointwise_equivalences.lp`;
...
the derived conventional restriction-equivalence and topology-locality
...
the universal-property localization-at-zero extension is
`emdash3_2_commutative_algebra_localization_zero.lp`;
the fixed-image localization-at-an-idempotent extension is
`emdash3_2_commutative_algebra_localization_idempotent.lp`;
...
the whole internal product/iterated-localization overlap-equivalence extension
is `emdash3_2_commutative_algebra_localization_overlap.lp`;
...
the supplied global projective-line presentation total is
...
the set-classified augmented semi-simplex face-map code extension is
`emdash3_2_semisimplicial_face_codes.lp`; raw indexed skip/keep syntax owns
structural composition, while public codes, constructors, and composition use
the existing 0-truncation reflector and add no proof-time unifier;
the internal augmented semi-simplex category is
`emdash3_2_semisimplicial_index.lp`; objects are finite vertex counts, Homs
are discrete path categories of face codes, and composition computes only at
visible public truncation points through the existing face-code owner;
the join-built ordinary simplex-shape and selected face-realization extension
is `emdash3_2_simplex_shapes.lp`; its whole join map reindexes the existing
cross cell, and its strict-profile dictionaries realize the two vertices of
the edge and three edges of the triangle without a generic face decoder;
the variable-dimension coherent-nerve level facade is
`emdash3_2_coherent_nerve_levels.lp`; it exposes `[n] -> C` as one whole
Functor category, adds an honest augmented empty shape at vertex count zero,
and retains generic transfor action without claiming face action or a whole
semisimplicial nerve;
the selected tetrahedral-coface extension is
`emdash3_2_tetrahedron_faces.lp`; four strict-profile triangle inclusions
compute from existing join maps, all six shared edges agree as face codes,
and the three new-vertex whole-functor comparisons remain explicitly gated by
generic join-map composition;
the first-stage directed-join mapping observation/extension interface is
`emdash3_2_join_mapping_recursion.lp`; whole branch restriction and the
action-derived internally natural cross observation are active, while its
explicit object-data classifier is not yet a Cat-valued coherent-square total
and therefore carries no broad join eta or mapping-category equivalence;
the whole primitive/action-derived join-cross compatibility extension is
`emdash3_2_join_cross_compatibility.lp`; one propositional higher-constructor
...
functors retain action on volumes and pair into one whole boundary functor,
its whole map is the first action of `Sigma(FF)`, and its iterable next action
sends a visible `(kappa,lambda)` tetrahedron to the existing
`fdapp1_int_hom_fapp0` projection without a new coherence record;
...
action, and uses typed stable-owner comparisons to expose a generic directed
cell between the two raw bracketings. Its Path specialization is invertible
without relying on `comp_assoc`, a new rule, or a new unifier;
the constructor-visible represented dependent-source extension is
`emdash3_2_dependent_simplex_represented_source.lp`; a three-edge Sigma spine
retains both bracketings, exposes the represented associator as the native
`(kappa,lambda)` tetrahedron input, maps it through the existing next internal
action, and keeps another hom action iterable without Sigma eta, a rule, or a
unifier;
the flagged native dependent-simplex dimensions and their recursively derived
whole map action are `emdash3_2_dependent_simplex_native_dimensions.lp`;
dimensions zero through three iterate `PathOut_cat`, visible triangles and
...
the intrinsically indexed flagged-code and category-decoder extension is
`emdash3_2_dependent_simplex_codes.lp`; each successor code stores one native
...
the variable-dimensional whole nonempty-face extension is
...
the constructed ordinal dimension-three continuation is
`emdash3_2_dependent_simplex_ordinal_dimension3.lp`; it realizes the one
canonical source tetrahedron through the shaped fixed-source `PathOut` and
whole post-laxity owners, maps that source under every
...
whole Pi sections, exposes a directed source comparison and computational
fibre component for recursive consumers, and retains another hom action. It
also exposes the whole constructor-visible source presentation and a coherent
expected-to-visible displayed reflag; their fibre beta turns normalized Pi
...
the fixed-boundary cubical square-total extension is
`emdash3_2_cubical_square_total.lp`; it totalizes the two side-arrow
coordinates of `homdc_`, exposes top, bottom, left, and right as whole
variance-correct observations, and maps a next-hom object to four side faces
...
Its generic varying-Sigma projection computes on point components and visible
arrows, including the pointwise-opposite specialization;
the variance-correct cubical internalization is
`emdash3_2_cubical_internalization.lp`; inner Sigma and pointwise opposite
make `homdc_int` a transparent specialization of the existing `homd_int`, and
the outer opposite Sigma derives `homdc_total_cat`. `LaxArrow_cat`, visible
edge/square constructors, and both endpoint functors add no primitive cubical
category, square, two-sided Sigma, rule, or unifier;
the directed cubical-arrow compatibility extension is
`emdash3_2_cubical_arrow.lp`; `CubicalArrow_cat`, its visible constructors,
and source/target are transparent aliases of that derived theory;
the readable cubical-arrow composition extension is
`emdash3_2_cubical_arrow_composition.lp`; square pasting post-whiskers the
first filler, pre-whiskers the second, and composes them in the ordinary Hom.
Those explicit identity/paste terms remain typed readings, while generic
nested-Sigma identity/composition owns the category normal forms and recovers
the side identities/composites. It adds no cubical associativity, filler, or
unifier;
...
the whole native semicubical nerve and its action laws are
`emdash3_2_semicubical_nerve.lp`; one functor from the opposite augmented
semicube category computes on objects to native cubical levels, while a typed
whole path compares its generic arrow action with the profile-retaining code
action. Restricted truncation proves public/category composition agreement;
generic functoriality then derives all-star identity and contravariant
composition without a competing runtime arrow beta or unifier;
the variable-dimensional immediate semicubical boundary is
`emdash3_2_semicubical_frames.lp`; a two-per-successor finite family begins
with the new `L/R` cofaces and recursively star-lifts every older face. Typed
public-decoder/action paths compare the new pair with native source/target and
the inherited family with profiled `CubicalArrow_func`; object projections
recover square and cube boundaries without a `Fin` type, dimension-specific
record, rewrite, or unifier;
the standard-representable semicube comparison is
`emdash3_2_semicubical_representables.lp`; Yoneda supplies
`StandardSemicube(n)`, its `p`-level is the path category of
`CubeFaceCode(p,n)`, and the whole native nerve action decodes those faces to
restriction functors with another Hom action retained;
the rule-free canonical cubical import facade is `emdash3_2_cubical.lp`;
...
action while the rigid public presheaf facade remains distinct;
the fixed dimension-two boundary/horn ordinary-sieve layer is
`emdash3_2_simplex2_sieves.lp`; omission bits compute from face codes, one
kind-indexed higher-sieve owner retains generic action, and existing sieve
pullback/extension/inclusion owners supply the categorical structure;
the bounded algebraic path-groupoid two-horn filler layer is
`emdash3_2_path_groupoid_2horn_fillers.lp`; inner composition, outer inverses,
J-derived cancellation, whole section paths, and iterable Path-map lifts are
active without claiming an all-dimensional nerve presheaf or Kan theorem;
the categorical semisimplicial decalage layer is
`emdash3_2_semisimplicial_decalage.lp`; its vertex-appending index shift,
whole base and cone-tip transformations, presheaf restriction, selected
levelwise cone fibres, and iterable base Path-map are active, while assembly
into one varying-`HFiber` displayed family and comparison with `homd_` remain
explicitly deferred;
...
3. `emdash3_2_fibrewise_sigma.lp` for the stable composite of a Cat-valued
   family `E : K -> Cat` and a second family over `Sigma(E)`, with computing
   fibres and one whole internally natural first projection. Explicit
   base-arrow beta and total-category Fubini comparison remain consumer-gated;
4. `emdash3_2_eq1_hom_action.lp` for the transparent derived native
   equality-valued
...
5. `emdash3_2_eq1_evidence_property.lp` for transparent native equality-valued
   evidence-property, retract-truncation, and finite-`NCat` object-truncation
   theorems;
6. `emdash3_2_sieves.lp` for native subterminal categories, ordinary sieves
...
7. `emdash3_2_nat_arithmetic.lp` for reusable Nat addition, associativity,
   Unit/Empty proposition evidence, and Nat sethood;
8. `emdash3_2_finite_families.lp` for Nat-indexed right-associated finite
...
9. `emdash3_2_commutative_algebra.lp` for set-carrier commutative-ring
...
10. `emdash3_2_commutative_algebra_category.lp` for structured ring morphisms,
   ...
   polynomial package;
11. `emdash3_2_commutative_algebra_product.lp` for the componentwise product
...
12. `emdash3_2_commutative_algebra_f2.lp` for the closed Boolean-carrier
...
13. `emdash3_2_commutative_algebra_finite.lp` for finite ring sums/dot
...
14. `emdash3_2_commutative_algebra_polynomial.lp` for free commutative
   ...
   eta;
15. `emdash3_2_commutative_algebra_localization.lp` for proposition-valued
   ...
   Zariski interface;
16. `emdash3_2_commutative_algebra_localization_unit.lp` for the canonical
...
17. `emdash3_2_commutative_algebra_localization_zero.lp` for derived
    ...
    overlap, or scheme;
18. `emdash3_2_commutative_algebra_localization_idempotent.lp` for the
...
19. `emdash3_2_commutative_algebra_localization_comparison.lp` for unit
    ...
    interface;
20. `emdash3_2_commutative_algebra_localization_overlap.lp` for
    ...
    first-class `OmegaEquiv` facade. It is rule-free and retains ordinary
...
21. `emdash3_2_commutative_algebra_presheaves.lp` for the transparent
   ...
   package;
22. `emdash3_2_sites.lp` for ordinary-sieve membership, the canonical maximal
...
25. `emdash3_2_commutative_algebra_locality.lp` for the coverhood view of the
   ...
   `Spec`, or scheme;
26. `emdash3_2_commutative_algebra_matching.lp` for the Path-valued carrier
...
27. `emdash3_2_commutative_algebra_glue.lp` for computational localization
...
28. `emdash3_2_commutative_algebra_affine_glue.lp` for the identity
...
29. `emdash3_2_commutative_algebra_zariski.lp` for selected finite families
...
30. `emdash3_2_commutative_algebra_localization_split.lp` for the generic
...
31. `emdash3_2_commutative_algebra_zariski_topology.lp` for the rule-free
...
32. `emdash3_2_commutative_algebra_affine_spec.lp` for the conventional big
...
33. `emdash3_2_commutative_algebra_affine_zariski.lp` for the rule-free
...
34. `emdash3_2_commutative_algebra_affine_points.lp` for the rule-free affine
...
35. `emdash3_2_commutative_algebra_affine_intersections.lp` for the rule-free
...
36. `emdash3_2_commutative_algebra_affine_atlas.lp` for the rule-free
    ...
    coordinate restrictions compute to whole maps into the zero ring. Its
...
37. `emdash3_2_ringed_sites.lp` for an assumption-explicit, rule-free
...
38. `emdash3_2_site_basis.lp` for the transparent rule-free whole restriction
    of value-category-valued presheaves along a base functor, a supplied
    sheaf-restriction functor connected to that operation by one whole
    `IsoEvidence`, and comparison-lemma strength as
    `OmegaEquivAlong Cat_cat`. Generic owners retain all arrow action and
    naturality. A proof-time path/`IsoEvidence` bridge compares generic
    precomposition's stable cut normal form with direct composition without
    adding a runtime fold. The module does not construct topology, continuity,
    sheafification, induced slices, a Beck--Chevalley mate, local-exactness
    evidence, component squares, or an equivalence of raw base categories;
39. `emdash3_2_commutative_algebra_ringed_space_covers.lp` for the rule-free
    global-first cover substrate. It retains one reflective CommRinged site,
    one distinguished object, and one ordinary covering sieve, while
    Grothendieck stability derives the pulled-back cover of each selected
    ...
    scheme;
40. `emdash3_2_commutative_algebra_binary_covers.lp` for the transparent
...
41. `emdash3_2_commutative_algebra_ringed_space_restrictions.lp` for the
...
42. `emdash3_2_commutative_algebra_affine_ringed_sites.lp` for the rule-free
    ...
    its included presheaf to the computing coordinate presheaf. The resulting
    reflective ringed site has the exact generated topology; readable chart
    components project the whole comparison and do not replace its internal
...
43. `emdash3_2_commutative_algebra_affine_locality.lp` for the rule-free,
...
44. `emdash3_2_commutative_algebra_affine_schemes.lp` for the rule-free thin
...
45. `emdash3_2_commutative_algebra_affine_basis.lp` for the transparent
    ...
    computing ambient restriction to the existing affine-scheme presentation;
    generic `DefIso` composition yields the coordinate comparison. It does
...
46. `emdash3_2_commutative_algebra_affine_cover_charts.lp` for the
...
47. `emdash3_2_commutative_algebra_affine_cover_presentations.lp` for the
...
48. `emdash3_2_commutative_algebra_affine_cover_refinements.lp` for the
...
49. `emdash3_2_commutative_algebra_local_ringed_sites.lp` for the transparent
    ...
    define a scheme;
50. `emdash3_2_commutative_algebra_locally_ringed_space_presentations.lp` for
...
51. `emdash3_2_commutative_algebra_site_relative_schemes.lp` for the
...
52. `emdash3_2_finite_limits.lp` for the transparent rule-free selected
...
53. `emdash3_2_commutative_algebra_scheme_chart_overlaps.lp` for the
...
54. `emdash3_2_commutative_algebra_laurent.lp` for the transparent rule-free
...
55. `emdash3_2_commutative_algebra_scheme_laurent_overlaps.lp` for the thin
...
56. `emdash3_2_commutative_algebra_projective_line.lp` for the transparent
    rule-free dependent total of one supplied global binary site-relative
    scheme, its actual selected chart intersection, and the existing Laurent
    presentation on the inherited structure-ring restrictions. It validates
    the end-to-end projective-line-style computational boundary without
    constructing `Proj`, gluing a global object from charts, adding external
    overlap/cocycle fields, or proving non-affineness;
57. `emdash3_2_walking_end_hit.lp` for the selected concrete walking-
    endomorphism directed-HIT/`BNat` model, eliminator, comparison, and
    directed negative results;
58. `emdash3_2_strict_pointwise_equivalences.lp` for generic strict
    pointwise-to-whole `OmegaEquivAlong` assembly over ordinary and displayed
    transformations. The forward transformation already carries naturality;
    rigid inverse owners compute to the selected component inverses, while
    whole cancellation paths complete the fixed-forward equivalence. It does
    not assemble a transformation from an incoherent component family or add
    generic functor extensionality, lax inversion, or a runtime univalence
    fold;
59. `emdash3_2_telescope_localization_hit.lp` for the set-truncated sequential
    telescope generated by one endomap, its dependent eliminator and point
    computation, the internally derived whole cocone universal property, and
    the computing shift inverse of the localized transition. It is a reusable
...
60. `emdash3_2_direct_cover_questions.lp` for the whole Path-valued family of
...
61. `emdash3_2_direct_cover_question_families.lp` for the whole varying
...
62. `emdash3_2_direct_cover_algebras.lp` for the transparent rule-free whole
...
63. `emdash3_2_direct_cover_internal_sheaves.lp` for the transparent rule-free
...
64. `emdash3_2_direct_cover_completion_hit.lp` for the first direct
    ...
    derived. The opposite restriction/glue law and conventional locality live
    in the subsequent comparison module. It deliberately does not itself
    claim whole Hom universality, reflector, rigid-sheaf-facade realization,
    CommRing lift, or left exactness;
65. `emdash3_2_direct_cover_completion_locality.lp` for the conventional
    comparison. One retained-member theorem derives
    `restriction(glue(m))(p,member)=m(p,member)` from canonical pullback,
    matching naturality, recursive glue, and silent. A rigid whole
    transformation projection tower internalizes that result; strict
    pointwise closure then gives `restriction o glue = id` and combines with
    silent to construct `IsTopologyLocalPsh(DirectCoverCompletionPsh)`. The
    module does not yet construct whole Hom universality, a reflector,
    rigid-facade realization, CommRing lift, or left exactness;
66. `emdash3_2_direct_cover_completion_eliminator.lp` for whole maps between
    ...
    projection to that algebra. Conventional locality of the completion is
...
77. `emdash3_2_gray_walking_square.lp` for the transparent rule-free
    ...
    two strict inner functors supplied by coevaluation; its inner arrows retain
    whole Terminal-indexed generator owners; and its outer arrows are
...
79. `emdash3_2_truncation_reflector.lp` for the classified computational
    ...
    action;
80. `emdash3_2_semisimplicial_face_codes.lp` for set-classified augmented
    ...
    claim;
81. `emdash3_2_semisimplicial_index.lp` for the internal augmented
    semi-simplex category. Objects compute to finite vertex counts and Homs to
    discrete path categories of face codes. Identity is the all-keep code;
    category composition reduces at visible public truncation points through
    `face_comp`, while arbitrary composition retains the generic owner. It
    adds no unifier, degeneracy, join realization, representable, sieve, or
    Kan claim;
82. `emdash3_2_simplex_shapes.lp` for ordinary Nat-indexed simplex shapes,
    generic whole join-map action, strict-profile join inclusions/maps, and
    the selected five cofaces through dimension two. Its cross datum is the
    target join's existing cross cell reindexed along both inputs; code and
    whole-functor coface equations compute without an external naturality
    family. It adds no augmented-empty realization, arbitrary face decoder,
    degeneracy, representable, sieve, or Kan claim;
83. `emdash3_2_coherent_nerve_levels.lp` for the variable-dimension mapping
    category `Functor_cat(DirectedSimplex_cat(n),C)` and its separately
    augmented vertex-count facade. The zero augmented shape is
    `Path_cat(Empty_grpd)` and successor levels recover the ordinary shape;
    generic Functor/Transf hom action is retained. It adds no face action,
    whole semisimplicial nerve, recursive-cell bridge, rule, or unifier;
84. `emdash3_2_tetrahedron_faces.lp` for the four selected strict-profile
    triangle cofaces of `Delta[3]`. All six shared edges compute through
    `FaceCode`; the three old-base edges also agree as composite whole
    functors, while the three new-vertex comparisons deliberately await a
    generic join-map-composition bridge. It adds no rule, unifier, generic
    decoder, or recursive top cell;
85. `emdash3_2_join_mapping_recursion.lp` for first-stage observation and
    extension of maps out of a directed join. Whole branch restriction
    functors retain transfor action, and the whole cross observation derives
    from the active profunctor action. Its explicit object-data Sigma is not a
    Cat-valued coherent-square total; no action-derived-cross beta, join eta,
    mapping-category equivalence, rule, or unifier is added;
86. `emdash3_2_join_cross_compatibility.lp` for the propositional whole beta
...
88. `emdash3_2_dependent_simplex_bridge.lp` for the transparent recursive
    ...
    fourth surfaces and pair as one product-valued boundary functor; a visible
    higher constructor computes through `fdapp1_int_hom_fapp0`, and every
    projection retains another hom action. It adds no standalone coherence
    cell, rule, unifier, `homd_con_int`, or whole mapping-category comparison;
89. `emdash3_2_semisimplicial_diagrams.lp` for standard Yoneda
    ...
    data;
90. `emdash3_2_simplex2_sieves.lp` for the boundary and three horns of the
...
91. `emdash3_2_path_groupoid_2horn_fillers.lp` for the bounded algebraic
...
92. `emdash3_2_semisimplicial_decalage.lp` for the vertex-appending shift on
...
93. `emdash3_2_circle_connectedness.lp` for the rule-free first concrete
...
94. `emdash3_2_walking_circle_restriction.lp` for the transparent rule-free
...
95. `emdash3_2_walking_circle_extension.lp` for the whole inverse candidate
...
96. `emdash3_2_walking_circle_universality.lp` for the rule-free concrete
...
97. `emdash3_2_walking_circle_monodromy.lp` for the rule-free universe-valued
...
98. `emdash3_2_groupoidal_interval_hit.lp` for the opaque two-endpoint
...
99. `emdash3_2_walking_interval_comparison.lp` for the concrete
...
100. `emdash3_2_walking_interval_restriction.lp` for whole precomposition of
...
101. `emdash3_2_walking_interval_extension.lp` for the whole inverse candidate
...
102. `emdash3_2_walking_interval_universality.lp` for the rule-free
...
103. `emdash3_2_groupoidification_hit.lp` for category-indexed
...
104. `emdash3_2_groupoidification_universality.lp` for whole restriction,
...
105. `emdash3_2_groupoidification_composition.lp` for the nonidentity generic
    unit compositor and one retained next action, both projected from the
    existing ordinary internal-action calculus;
106. `emdash3_2_groupoidification_interval_recovery.lp` for the two extension-
...
107. `emdash3_2_dependent_simplex_path_associator.lp` for the rule-free
    ...
    retains another hom action. Typed endpoint paths yield a generic directed
    `(h o g) o f -> h o (g o f)` cell; in the literal Path profile it is an
    invertible equality. The construction neither uses `comp_assoc` nor
    identifies its Path proof with the separate direct-J associator;
108. `emdash3_2_dependent_simplex_represented_source.lp` for the rule-free
    constructor-visible three-edge Sigma spine. Its represented directed
    associator is re-presented as the native dependent tetrahedron, the
    existing next internal action computes both projected components, and a
    further hom action remains available. It adds no Sigma eta, rule, unifier,
    complete boundary telescope, or uniform dependent-simplex code;
109. `emdash3_2_dependent_simplex_native_dimensions.lp` for the rule-free
    flagged classifiers through dimension three. Each successor is a
    `PathOut_cat`, `pathout_map_func` and its iterations give whole map action,
    visible dimension-two and dimension-three constructors expose every face
...
110. `emdash3_2_dependent_simplex_dimension4.lp` for the rule-free fourth
...
111. `emdash3_2_dependent_simplex_codes.lp` for the curated intrinsically
    indexed flag grammar. Zero is indexed by `C`; successor is indexed by
    `PathOut_cat(K,x)`. The existential package decodes by category projection,
    selected codes recover native dimensions zero through four, existing
...
112. `emdash3_2_dependent_simplex_code_map.lp` for functorial mapped decoding
...
113. `emdash3_2_dependent_simplex_faces.lp` for whole nonempty-face action on
...
118. `emdash3_2_dependent_simplex_ordinal_dimension3.lp` constructs the
    ...
    component, and another hom action remain public. Its Path specialization
...
121. `emdash3_2_prof_reindex_terminal_normalization.lp` for whole
    ...
    betas expose its rigid Hom action, whole displayed normalizer, and
    expected-to-constructor-visible reflag, while generic
...
123. `emdash3_2_cubical_square_total.lp` for the fixed-boundary nested Sigma
...
124. `emdash3_2_cubical_internalization.lp` for transparent
...
129. `emdash3_2_cubical_square_level.lp` for the first iterated arrow level.
    ...
    unifier;
130. `emdash3_2_cubical_levels.lp` for genuine Nat recursion from `C` by
...
131. `emdash3_2_semicubical_face_codes.lp` for intrinsically indexed raw
    ...
    proof-time unifier;
132. `emdash3_2_semicubical_index.lp` for the internal augmented semicube
...
133. `emdash3_2_semicubical_face_action.lp` for variable-dimensional native
...
134. `emdash3_2_semicubical_nerve.lp` for the whole functor
...
135. `emdash3_2_semicubical_frames.lp` for the generic immediate boundary.
...
136. `emdash3_2_semicubical_representables.lp` for Yoneda standard semicubes,
    their whole native decoder, selected comparison with the computing face
    action, and retained higher representable action;
137. `emdash3_2_cubical.lp` for the rule-free canonical import facade;
138. `emdash3_2_checks.lp` for executable regression statements;
139. `reports/REPORT_EMDASH_V3_2_CURRENT_STATUS_AND_SOP_2026-05-26.md`
    for current architecture and development SOP;
140. `reports/EMDASH_FOUNDATIONS.md` for the mathematical reading;
141. `reports/REPORT_EMDASH_V3_2_CANONICAL_SURFACE_SYNTAX_2026-06-05.md`
    for comment/example notation;
142. `reports/INDEX.md` for task-specific plans and decision records;
143. `book/book.json` and `book/evidence.json` for book source
