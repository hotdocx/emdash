...
| `STDLIB-8B` | PathOut/PathInd trusted profile and derived public library | in progress | `PATHOUT-TRUST-BOUNDARY-0A`, root-only `PATHOUT-LIBRARY-FOUNDATION-1B`, fixed-source `PATHIND-TRUSTED-PROFILE-1C` at `a361dc3`, generic checkpoint `e560551`, root-only internalized `PATHOUT-LIBRARY-INTERNALIZED-1D` at `b6005b3`, and root-only transitivity `PATHOUT-LIBRARY-TRANSITIVITY-1E` are complete. Reviewed transitivity v4 at `2498053`/`fc9a323` closes exact 0/1/0/5: eight TypeScript observations, two consumers, eight negatives, authority closure, and eight separately gated Lambdapi assertions are green. Its semantic checkpoint awaits only ledger pinning. Public presentation `1F` is next; graduation `0G` remains later. |
...
| `D-PA-174` | Complete root-only `PATHOUT-LIBRARY-TRANSITIVITY-1E` at reviewed v4 and advance only to presentation `1F`. | The transfer compiles five definitions and one subject-checked original-consumer-parent support at exact 0/1/0/5. Cold focused execution passes eight active tests with zero failures and one ordinary oracle skip; all observations, both consumers, eight negatives, inherited proof reuse, canonical predecessor-name repair, rule-ID selection, authority closure, and non-export checks are green. The separately gated run accepts all eight active-Lambdapi assertions under a 20-second ceiling with warnings disabled. Root typecheck, focused lint, diff hygiene, and a clean generic-compiler diff pass; the `e560551` aggregate is carried forward. No generic engine, Core, new proof rule, active Lambdapi source, or public/package surface changes. The exact semantic checkpoint is pinned by the immediately following ledger-only update. |
