...
checkpoint `fe1a9b7` is separately reviewed under delegated unattended
authority, so only that exact root-local implementation may now resume.
...
| `STDLIB-8B` | PathOut/PathInd trusted profile and derived public library | in progress | `PATHOUT-TRUST-BOUNDARY-0A`, root-only `PATHOUT-LIBRARY-FOUNDATION-1B`, fixed-source `PATHIND-TRUSTED-PROFILE-1C` at `a361dc3`, generic checkpoint `e560551`, and root-only internalized `PATHOUT-LIBRARY-INTERNALIZED-1D` at `b6005b3` are complete. Reviewed v14 `4244b54`/`73aa94e` seals 4/13/0/10; the semantic checkpoint passes its focused and separately gated oracle evidence. Transitivity v1 admitted all five definitions but exposed two presentation residuals. V2 proposal/review `b1e6f0f`/`31f23db` freeze 0/1/0/5; cold v2 proves its exact pre-delta rule is shadowed despite subject checking and isolated firing. V3 proposal `fe1a9b7` replaces it one-for-one with the measured post-delta parent and retains explicit descendant-environment proof reuse; its separate delegated review authorizes only that exact implementation. Public presentation `1F` remains later. |
...
| `D-PA-171` | Approve only corrected post-delta proposal checkpoint `fe1a9b7` under delegated unattended authority, with human supersession. | The separate review pins proposal SHA-256 `0d7448ae68d9aa6ae3bf91b9010a676f8ca3c9101976e1de2c88816a94e68dd9` and copies exact 0/1/0/5 implementation and observation boundaries. It authorizes one stable post-delta replacement, explicit descendant-environment rechecking of inherited `stress.sigma-pi.uncurrying`, and nothing else. It denies retaining v2, a second rule, broad `hom_con` or `id_funcd` import, new proof/runtime-category authority, generic-engine/Core/public/Lambdapi changes, integration, and release. Forty-one combined tests, root typecheck, focused lint, non-export checks, and diff hygiene are green; no aggregate is rerun. |
