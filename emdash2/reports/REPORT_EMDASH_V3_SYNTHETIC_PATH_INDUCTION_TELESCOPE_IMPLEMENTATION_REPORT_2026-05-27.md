...
## 2026-05-30 Consolidated Status: Goal, Achievements, And Scope

The core milestone is achieved in the intended synthetic/telescope form.
...
This type already internalizes naturality in the varying source object `x`.
There is no separate external naturality square to prove by hand for the core
milestone. What must compute, and now does at the inspected surfaces, are the
source and target transports induced by an arrow `p : x -> y`:
...
The target transport is supplied by the Pi/naturality-laxity projection:

```text
functord_laxity_transf(PathOutPi_funcd(Z),p)[E]
  -> PathIndTgt_transport_func(p,E)
  -> section_pullback_func(PathOut_transport(p),E).
```

This target-side computation is shared infrastructure for both presentations:
the telescope theorem and the derived Sigma-total theorem.

The achieved computational regressions include:
...
The Sigma-total theorem is intentionally a derived uncurrying:

```text
PathInd_funcd(Z) = Sigma_transfd_funcd(PathInd_transfd(Z)).
```

For the current milestone, the Sigma formulation is only required to compute
over canonical/cartesian total arrows generated by transport of `(x,E)`.
Arbitrary non-cartesian Sigma-arrow naturality remains deferred. This is not a
gap in the telescope theorem: it is a broader Sigma/omega action problem.

The recent laxity work was triggered by the target transport problem above.
For arbitrary displayed `FF`, the image of a canonical/cartesian total arrow
under `Sigma(FF)` exposes the displayed laxity component:
...
The active Sigma-map normal form therefore consumes laxity rather than assuming
strict/cartesian preservation:
...
Remaining work should be read in that light: improve and generalize the
lax/strict infrastructure only when a concrete downstream computation needs
it, rather than reopening the already achieved path-induction/transitivity
milestone.
...
- The Pi pullback laxity projection is already implemented at both the
  whole-transfor and component levels:

  ```text
  functord_laxity_transf(Pi_pullback_funcd(G),p)
    -> section_pullback_transf(G[p])

  functord_laxity_transf(Pi_pullback_funcd(G),p)[E]
    -> section_pullback_func(G[p],E).
  ```

  What remains deferred is only the separate Sigma-total transport presentation
  for `Sigma_catd_functord_catd(Pi_pullback_funcd(G))`. If a downstream theorem
  requires that exact surface, keep it as a focused total-action theorem rather
  than a direct rule on the transparent `Sigma_catd_transport_func` alias.
...
- Introduce explicit strict wrappers only as a separate cleanup/generalization
  phase. The v2-style plan is still relevant:
  ...
  The active v3.2 file still has temporary global strict functoriality and
  strict naturality rules. Migrating those behind explicit strict structure is
  important, but it is not a prerequisite for the achieved telescope/Sigma
  path-induction regressions.
