...
| `CGCA-AUDIT-1` | in progress | Re-audit the Yoneda, Gray profile/closure, walking-square/interchanger, native lax-arrow, and recursive cubical owners against the literature-backed distinction above. Record exact orientation and missing action owners before semantic edits. |
| `CGCA-YONEDA-2` | pending | Implement `cubical_yoneda_section` and `cubical_yoneda_eval` through existing `fib_cov_transf` and fibre evaluation; validate arbitrary `C,n` and visible dimensions without a new naturality family. |
| `CGCA-YONEDA-BETA-3` | pending | Establish the selected identity-face beta, judgmentally if supported or by one typed path at the existing owner. Retain another Hom action and defer full eta/equivalence. |
| `CGCA-GRAY-ORIENT-4` | pending | Determine whether the active `GrayTensor_R(I,I)` interchanger is `b o u ==> v o a`, its reverse, or only propositionally readable. Select factor order/opposite/profile naming from a typed dimension-two boundary probe; add no endpoint rewrite. |
