...
account below distinguishes retained ordinary references from the native route.
...
kernels/cokernels, exactness, and homology require additional capability-indexed
structure.
...
### Current Universal And Homological Interfaces

The finite-free weak-kernel and Freyd algorithms remain useful computation
owners, with their selected syzygies, factor witnesses and presentation data.
Their mathematical applicability is capability-indexed. Arbitrary CommRing
structure alone does not provide a closed effective quotient model or an
algorithm for every universal construction. Raw equations, quotient equality,
all-test provider semantics and whole-model interpretation are distinct
contracts. The CAS engine cannot infer the latter from a finite list of
successful matrix equations.

The primary homological interface in an ordinary additive target C starts
with its actual walking-arrow diagram category. Write J(A)=(A→0) and
I(A)=(0→A). Whole selected functors K and Q carry adjunctions J⊣K and Q⊣I.
The counit supplies the kernel inclusion; the unit supplies the cokernel
projection. Their whole Hom mate operations construct lifts and descents.
...
K(h)∘η_A. Applying Q to its whole arrow-diagram family gives H. The same
construction at the universal input gives H on the existing native zero-cone
category, whose carrier and Homs come from the original internal dependent
Homs. H maps are whole transformations obtained by applying Q to boundary
maps. No replacement hom_int/homd_int foundation or new primitive H is used.

The canonical Coim⇒Im comparison is constructed before normality is supplied.
Normality concerns that actual transformation and retains its Ω inverse data.
Exactness likewise concerns the canonical Im(f)→Ker(g) comparison for an
actual zero-composite input. This avoids replacing a selected comparison by
an unrelated isomorphism between similarly named objects.

The connecting transformation δ is direct: it uses original kernel lifts,
quotient descents and derived covers, without selecting a splitting of a
short-exact row. All three exactness positions of the native homology window
are derived from the original data. The native snake retains arbitrary outer
maps a and c and all six kernel/cokernel terms; its four interior exactness
results and comparison with the native LES, including the connecting-map
sign, are separately qualified. Neither construction requires the retired
snake-derived connecting implementation.

The native proof–CAS model uses these whole K/Q structures directly. Model,
normality and interpretation contracts remain supplied explicitly; displayed
output exactness is derived. The bounded nonsplit LES and corresponding snake
have checked displayed certificates. The worked polynomial example, its raw
matrices, retained redundant presentation relations, and exact source/trust
boundary are in
[Chapter 31](../book/chapters/31-additive-abelian-and-homological-computation.md#homology-nonsplit-worked-example).
Computed equations do not constitute a closed construction of every model
contract. Selected inverse and matrix data are not replaced by opaque proofs.

Current source boundaries are explicit:

- shared raw pair/map input vocabulary is independent of former H algorithms;
- ordinary H records are independent downstream observations;
- the superseded model facades and snake-derived connecting closure are
  retired, with source preserved by Git history;
- shared CAS/provider algorithms and the ordinary finite-window iterator
  remain useful references, without an old/new compatibility obligation;
- finite point interpretation equations remain valid observations of whole
  operations; the generic whole input/H comparison is a retained unqualified
  follow-up rather than an implemented interface.

This is a computational categorical reference development with explicit
exactness evidence and model contracts. It does not establish a general
Došen-style homological normalization theorem. The higher terminality,
Op/profile, six-term comparison and further derived/stable constructions keep
their recorded qualifications and deferrals. Current evidence is routed by the
[native final audit](../../docs/TYPESCRIPT_EMDASH_NATIVE_UNIVERSALITY_FINAL_AUDIT.md)
and the [consolidation plan](../../docs/TYPESCRIPT_EMDASH_CATEGORICAL_CORE_CONSOLIDATION_REVIEW.md).
The earlier detailed progression is preserved in the
[history report](REPORT_EMDASH_V3_2_CONSOLIDATED_HISTORY_2026-09-16.md#earlier-foundations-algebra-and-homology-narrative),
including its now-superseded claims about unfinished work.
