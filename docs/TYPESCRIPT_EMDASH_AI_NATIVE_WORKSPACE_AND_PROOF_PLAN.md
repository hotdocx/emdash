...
AI-SYNTH-1B0's authoring-consumer audit is complete and its frozen
AI-SYNTH-1B1 leading-implicit host macro is final-green; reusable scopes and
...
The completed AI-SYNTH-1B0 audit partitions the authoring/workspace row.
AI-SYNTH-1B1 is one direct-TypeScript host macro that derives the target of a
global declaration's first implicit Pi binder from an already checked prefix,
runs 1A over a call-site-explicit candidate list, and emits one ordinary
explicit transfer argument. The macro and its trace erase before the final
...
The completed AI-PAPER-0 audit selects the authored overview source
`emdash2/print/public/emdash-v3-2-overview.md` and registered document ID
`emdash-v3-2-overview` as the first real document consumer. The tracked
`docs/emdash3_2.md` file is a promoted copy and is never edited as source.
The print pipeline's date/counter-derived DOM IDs are render-local and cannot
be semantic block identities.

AI-PAPER-1 is therefore partitioned as follows:

1. AI-PAPER-1A is a browser-safe semantic binding. It validates one stable
   document identity and supplied source digest, stable block identities,
   diagram-to-declaration references, and proof blocks whose exact expected
   module/declaration identity and current proof fingerprint agree with an
...
3. AI-PAPER-1C is partitioned after exact-consumer audit. AI-PAPER-1C1 adds a
   browser-safe replay from release pins in the typed management sidecar and a
   lazy explicit-action panel in the existing external-reviewer webpage. Its
   release test requires byte equality with the Node-verified 1B1 binding.
   Inline PDF/Markdown placement is separately consumer-gated as 1C2. Neither
   route may create proof authority, replace stable source IDs with transient
   DOM IDs, or mutate article prose by implication.
4. Hosted synchronization, snapshots, builds, and publication remain a
   separately gated platform adapter owned by `~/closerfans`. The local file
   contract must be qualified before that repository changes.

Reject or split 1A if it imports Node builtins, parses Markdown, hashes bytes,
stores a checker/session, accepts a caller-asserted proof status, silently
...
| AI-PAPER-1C | partitioned | AI-PAPER-1B1 plus exact browser/print consumers | Separate a release-pinned browser recheck/panel from any inline print projection. |
| AI-PAPER-1C0 | complete | AI-PAPER-1B1 plus browser reviewer and print pipeline | The existing lazy browser bridge is the smallest real supplement consumer; print/PDF insertion lacks an inline-placement consumer and remains deferred. |
...
| D-AI-NATIVE-068 | accepted after AI-SYNTH-1B0 audit | The private `categorical_text` parser is a category-specific optional input adapter, not the generic declaration-management or dictionary-synthesis boundary. AI-SYNTH-1B must not add a parser production or category-specific resolver case. |
| D-AI-NATIVE-069 | accepted after AI-SYNTH-1B0 audit | Ordinary `CoreChecker` generic implicit insertion creates equation-solving metas, while global transfer calls are intentionally positionally explicit before checking. Dictionary authoring must expand to an explicit transfer argument before module compilation rather than teach the checker to search declarations. |
| D-AI-NATIVE-070 | accepted and frozen for AI-SYNTH-1B1 | The first authoring input is a checked earlier-fragment declaration context, one exact installed free-declaration callee whose checked type begins with an implicit Pi, and 1A's complete explicit finite candidate list. The target is that direct leading binder type; transparent-head reduction and later/local binders are deferred. |
| D-AI-NATIVE-071 | accepted and frozen for AI-SYNTH-1B1 | Success returns a deeply frozen ordinary `CoreLfTransferArgument` whose implicit value is the selected qualified global, together with the complete 1A result. Missing and ambiguous selection remain 1A structured errors; the caller never supplies the binder target or selected value. |
...
| D-AI-NATIVE-084 | accepted after AI-PAPER-1C0 audit | The first presentation consumer is the existing `emdash-template` external-reviewer webpage, through a new lazy additive bridge. The frozen minimal `browser.ts` entry remains byte-identical. The print/PDF pipeline remains untouched until inline placement is actually requested. |
| D-AI-NATIVE-085 | accepted and qualified by AI-PAPER-1C1 | Extend the typed management sidecar with exact release pins for the whole authored article, proof profile, and each canonical proof artifact. AI-PAPER-1B1 must verify every pin from bytes before a browser release can claim parity. Status and goals remain derived rather than pinned. |
...
test evidence. At this checkpoint AI-SYNTH-1B remained gated on an exact
authoring/workspace consumer; the subsequent 1B0 audit below selects that
consumer. AI-SYNTH-2 remains gated on a genuinely recursive indexed consumer.

## AI-SYNTH-1B0 Inventory And AI-SYNTH-1B1 Exact Contract

Date: 2026-08-08

The read-only authoring audit found:

- `CoreChecker.inferGenericCall` already inserts ordinary implicit
  metavariables when an explicit argument skips an implicit Pi binder. Those
  metas are intentionally constrained only by bidirectional checking and must
  be solved before the public boundary returns. The checker has no declaration
  candidates, instance scope, or reason to reinterpret an unconstrained meta
  as a dictionary request.
- The generic transfer IR is already the canonical portable source language.
  Its global calls carry every argument and plicity explicitly, and the
  declaration compiler validates a known global call positionally before the
  checked Core call is constructed. Relaxing that validation would merely
  create an unresolved meta for an unconstrained dictionary and would weaken
  the explicit source contract.
- `categorical_text.ts` is a private, expected-classifier-guided adapter for
  the reviewed category-specific program. It neither manages generic LF
  declarations nor produces `CoreLfModuleSpec`, so extending its parser or
  head resolver would recreate the wrong architectural dependency.
- Direct TypeScript host macros are an established user-facing boundary. The
  structure and adjunction macros inspect typed host data and erase to
  ordinary transfer declarations/rules before `CoreLfModuleSpec`. A dictionary
  authoring helper can follow the same pattern without adding a syntax or Core
  node.
- The exact checked prefix authority is already
  `CoreLfMixedDeclarationBaseContext`. The same-module fragment workspace can
  compile a prefix, let management code construct a later explicit fragment,
  and then reconstruct and compile the complete canonical plan with exact
  `earlier-fragment` provider identities. Its source snapshot stores only the
  resulting explicit module data, never a process-local checker or macro
  result.
- Workspace `externalProviders`, source order, linkage spelling, and the set of
  declarations in an environment are not instance annotations. Reusing any of
  them as an ambient candidate registry would violate 1A's complete explicit
  scope and existing visibility/provider meanings.

The selected consumer is a two-fragment direct-TypeScript module. Its first
declaration fragment installs a dictionary carrier, one or two ordinary
global dictionaries, and `useDictionary : Π [dictionary : Dictionary],
Dictionary`. Management code compiles that prefix and invokes the authoring
helper without supplying a target or dictionary term. The later fragment
defines an explicit body using the generated implicit argument and names the
carrier, callee, and selected dictionary as exact `earlier-fragment`
externals provided by the first fragment. Recompiling the full plan must check
that body and preserve the selected global in canonical source.

### Frozen AI-SYNTH-1B1 API and trust boundary

The browser-safe module is `src/v3_2/lf_dictionary_authoring.ts`, with profile
revision `emdash-lf-dictionary-authoring-v1`. Its operation
`synthesizeCoreLfLeadingDictionaryArgument` accepts only:

1. one checked `CoreLfMixedDeclarationBaseContext` from an already compiled
   prefix;
2. one exact qualified callee; and
3. one complete finite list of exact qualified candidates for this call site.

The callee must resolve to the same exact qualified identity, have an
installed free-declaration link, and have a checked type whose unreduced head
is an implicit Pi. The operation passes that leading binder type to
AI-SYNTH-1A. The caller cannot assert the target, selected Core name, selected
type, provider, priority, or success state. Transparent aliases whose checked
type does not directly expose the Pi, a nonleading implicit, and a local
binder are explicit later cases rather than hidden conversion/search.

Success returns a deeply frozen record containing the profile revision,
callee identity, binder name, the complete frozen 1A synthesis result, and
exactly this ordinary transfer argument:

```text
{ plicity: "implicit",
  value: { tag: "global", symbol: selectedQualifiedSymbol } }
```

The operation does not build or mutate a module, append an argument to a call,
compile a fragment, retain a checker, write a trace, or grant source
availability. The management program places the returned argument in an
ordinary transfer call. `createCoreLfModuleSpec`, linkage, external-provider,
visibility, declaration-compilation, and final checker boundaries then run
unchanged. Missing and ambiguous candidates remain the structured 1A errors
with complete reports; callee absence, wrong linkage/status, and absence of a
direct leading implicit binder are separate structured authoring errors.

The final canonical workspace snapshot contains the explicit selected global
and no synthesis node, callback, target assertion, registry, or process-local
state. Looking at the TypeScript management source still exposes the synthesis
request and exact candidate scope, while executing it produces independently
checkable derived evidence. This preserves the declared-source versus verified-
derived-state distinction.
...
same-module fragment-workspace suite, this ledger, and proportional TypeScript
checks. It authorizes no generic transfer/compiler/checker change, public or
browser barrel change, main-runner change, workspace snapshot revision,
parser, kernel/Lambdapi edit, platform integration, long unchanged aggregate,
external Git mutation, or publication.

### AI-SYNTH-1B1 Completion Record

Result: accepted and final-green for the erased leading-implicit host macro.

Implementation:

- added browser-safe `src/v3_2/lf_dictionary_authoring.ts` with profile
  `emdash-lf-dictionary-authoring-v1`;
- resolved and validated one exact installed global callee from a checked
  prefix, required its checked type to expose a direct leading implicit Pi,
  and derived the target solely from that binder;
- delegated the complete explicit candidate list and target to the final-green
  1A resolver without catching or weakening its missing/ambiguity reports;
- returned a deeply frozen ordinary implicit `CoreLfTransferArgument` whose
  value is exactly the selected qualified global, plus the complete 1A result,
  while preserving all caller inputs; and
- added no transfer-expression variant, synthesis placeholder, registry,
  workspace field/revision, parser route, checker/compiler behavior, Core
  node, runtime/proof rule, I/O, or backend selector.

The already registered same-module fragment-workspace suite now compiles a
checked prefix containing `Dictionary`, two global dictionaries, and one
leading-implicit consumer. It authors a later transparent definition from the
generated argument, requires exact earlier-fragment externals and provider
identities, recompiles the canonical two-fragment workspace from permuted
input, and confirms that checked Core and portable source retain the selected
global while containing no authoring/synthesis artifact. The same consumer
checks nonmutation/deep freezing, two-candidate ambiguity with the full 1A
report, and rejection of a callee without a direct leading implicit Pi.

Qualification:

- the focused same-module fragment-workspace suite passed 8/8 tests with zero
  failures;
- the nearest fragment-workspace, structure/resolver, generic declaration-
  compiler/migration, and declaration-workspace matrix passed 39 tests across
  five suites: 38 active passes, one intentional Lambdapi opt-in skip, and
  zero failures;
- workspace contract, root TypeScript typecheck, changed-file ESLint,
  browser-safety/forbidden-effect scan, and diff hygiene passed; and
- no shared aggregate boundary changed. The recent 1,486-test root aggregate
  remains valid under D-AI-NATIVE-048 and was not rerun.
...
The exact-consumer audit found:

- `emdash2/print/public/emdash-v3-2-overview.md` is the authored source for
  registered document `emdash-v3-2-overview`; `docs/emdash3_2.md` is its
  byte-identical promoted distribution copy;
- the current print pipeline extracts inline Arrowgram JSON but assigns
  render-local date/counter-derived DOM IDs, so neither those IDs nor diagram
  ordinals can be stable semantic references;
- `CoreProofArtifact` already owns exact module/declaration identity,
  complete versus incomplete state, named goals, checked Core only on
  completion, and source/profile/dependency fingerprints, while
  `assertCoreProofArtifactCurrent` owns freshness revalidation;
- Arrowgram's file-first `paper.md`/`diagram.json` convention is useful for a
  later local adapter, but `~/closerfans` already owns hosted workspace mounts,
  synchronization, Git snapshots, builds, authentication, and publication;
  and
- high-quality article prose should not acquire inline developer status data
  merely to support machine linkage. Stable source markers and a sidecar are
  the appropriate later file-level join.

Result: AI-PAPER-1A is accepted and final-green as the browser-safe semantic
binding, while exact-byte materialization and rendering remain separate.

Implementation:

- added `src/v3_2/research_document.ts` with profile
  `emdash-research-document-binding-v1`;
- validates stable document and block IDs, a canonical document revision,
  exact content references with syntactically valid supplied SHA-256 stamps,
  and a nonempty document-order block list;
- binds one Arrowgram block to a nonempty, duplicate-free canonical list of
  exact module/declaration references;
- binds one proof block to an exact expected module/declaration pair, an
  ordinary proof artifact, its newly supplied current fingerprint, and a
  stable external artifact reference;
- reuses `assertCoreProofArtifactCurrent`, independently preserves tuple
  identity rather than concatenated-name identity, and derives status,
  checked Core, and cloned stable named goals solely from the artifact;
- projects complete artifacts with checked Core and no goals, and incomplete
  artifacts with named goals and no checked Core; and
- returns a deeply frozen deterministic snapshot explicitly labelled
  `digestVerification: not-performed`, because hash computation and exact-byte
  verification remain AI-PAPER-1B.

The already registered proof-document suite uses the real article document
ID/path and its PathOut diagram declaration owners, then binds the existing
complete and intentionally incomplete TypeScript proof artifacts as Section 7
supplement identities. It confirms byte-stable serialization, canonical
diagram references, intentional complete/incomplete projections, deep
freezing, exact tuple identity, and rejection of duplicate blocks, malformed
digests, identity mismatch, and stale fingerprints. These test identities do
not yet claim that source markers or sidecar files exist; that is precisely
AI-PAPER-1B's consumer gate.

Qualification:

- the focused proof-document suite passed 8/8 tests with zero failures;
- the nearest proof-plan, proof-document, local proof-command, and exact-
  closure workspace-proof matrix passed 26 tests across four suites with zero
  failures or skips;
- workspace contract, root TypeScript typecheck, changed-file ESLint,
  browser-safety/forbidden-effect scan, and diff hygiene passed; and
- no public barrel, main test-runner, checker, compiler, runtime, package,
  print, book, or kernel boundary changed. The recent 1,486-test aggregate
  remains valid under D-AI-NATIVE-048 and was not rerun.

No authored or promoted article, print registry/pipeline, file below
`emdash2/`, or file in `~/closerfans` changed. Decisions D-AI-NATIVE-074
through D-AI-NATIVE-078 now have direct audit or implementation evidence.
AI-PAPER-1B remains gated on explicit source markers, fixed sidecar/artifact
...
## AI-PAPER-1C0 Audit And AI-PAPER-1C1 Completion Record

Date: 2026-08-08

The exact presentation-consumer audit found:

- `emdash-template` is the existing external-reviewer product and already
  owns a deliberately lazy bridge into reviewed root TypeScript modules. It
  is therefore the smallest real consumer of a checked/open paper supplement;
- the frozen minimal `src/v3_2/browser.ts` entry is a separate public evidence
  boundary and does not need to widen for this additive reviewer feature;
- a browser cannot perform the Node-owned fixed-file acquisition and hashing
  from AI-PAPER-1B1, but it can freshly replay the same inert proof plans from
  source-visible release pins; and
- the authored Markdown and PDF renderer currently have no explicit inline
  placement consumer for these supplemental proof states. Editing article
  prose or inventing a generated asset would therefore widen ownership rather
  than satisfy a measured requirement.

Result: AI-PAPER-1C0's consumer selection and AI-PAPER-1C1's release-pinned
client recheck are accepted and final-green. Inline print placement remains
AI-PAPER-1C2 and hosted synchronization remains AI-PAPER-1D.

Implementation:

- strengthened the typed management sidecar with exact whole-article,
  proof-source, proof-profile, and canonical proof-artifact SHA-256 pins;
- strengthened the existing Node materializer to verify all of those pins
  from exact UTF-8 bytes. It still diagnoses malformed diagram selection
  before the final whole-article pin, then fails closed on any remaining
  article drift before constructing a verified wrapper;
- added browser-safe `src/v3_2/ai_research_overview_browser.ts` with profile
  `emdash-ai-research-overview-browser-v1`. It performs no hashing, file I/O,
  network access, or retained checker/session work: it freshly compiles the
  two fixed proof plans from the pinned fingerprint and constructs the same
  ordinary AI-PAPER-1A document binding;
- made exact canonical binding equality between that browser recheck and the
  Node byte-verified AI-PAPER-1B1 materialization a focused release contract;
  neither proof status nor goal text is copied into the management sidecar;
- added one lazy `emdash-template` bridge and an explicit reviewer action. Its
  projection labels client recheck versus external Node verification, reports
  `complete_identity` as checked, and keeps `open_identity` visibly
  incomplete with stable goal `body`; and
- retained ordinary HTML prose and source IDs. No renderer-generated DOM ID
  becomes semantic identity, and no incomplete artifact is presented as a
  theorem.

Qualification:

- the focused proof-command/materializer file passed 12 tests across two
  suites, including exact browser/Node binding parity;
- the focused browser-reviewer file passed 11 tests in one suite, including
  a transitive Node-free closure check and lazy-shell source contract;
- together with proof plans, proof documents, and exact-closure workspace
  proof attachment, the proportional nearest matrix passed 44 tests across
  six suites with zero failures or skips;
- root workspace contract, TypeScript typecheck, changed-file ESLint, exact
  source-pin comparison, and diff hygiene passed;
- a fresh standalone fixture `npm ci` used the retained lock without changing
  its SHA-256, and the fixture TypeScript check plus Vite production build
  passed with 154 transformed modules. The new proof supplement remained its
  own lazy 24.45 kB chunk before gzip; the pre-existing large-chunk advisory
  and npm audit report were recorded rather than auto-fixed; and
- no public barrel, main test runner, generic checker/compiler/runtime,
  package manifest, lockfile, authored/promoted article, PDF, print pipeline,
  kernel, or hosted platform boundary changed. The recent 1,486-test root
  aggregate remains valid under D-AI-NATIVE-048 and was not rerun.

No file below `emdash2/`, generated `dist`, promoted publication artifact, or
file in `~/closerfans` changed. Decisions D-AI-NATIVE-084 through
D-AI-NATIVE-088 now have direct audit or implementation evidence.
AI-PAPER-1B2/1C2 remain gated on a real persistence or inline-placement
...
Treat AI-SYNTH-1B0 and final-green AI-SYNTH-1B1 as complete. Preserve its
direct leading-binder target derivation, call-site-complete finite scope,
...
the authored overview versus promoted-copy distinction; stable tuple
document/block/declaration identities; current ordinary proof-artifact
revalidation; intentional complete/incomplete projections; the typed source-
visible management plan; unique content-hash diagram selection; exact whole-
article/proof/profile/artifact release pins; the explicit inner
`not-performed` versus outer `performed-exact-utf8` digest boundary; and exact
canonical parity between Node verification and the lazy browser recheck.
Preserve 1A's focused 8/8 and nearest 26-test matrices, 1B1's prior focused
10-test/two-suite and nearest 31-test/five-suite matrices, and 1C1's final
44-test/six-suite nearest matrix plus standalone reviewer build; do not repeat
the unchanged 1,486-test aggregate. AI-PAPER-1B2/1C2 require a real persisted-
artifact or inline-placement consumer. Do not change article prose, print
