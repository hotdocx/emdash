...
prerequisites=(emdash3_2_homology_window.lp)
owners=(
  emdash3_2_homology_boundary_naturality.lp
  emdash3_2_cokernel_record_covers.lp
  emdash3_2_homology_map_kernel_covers.lp
  emdash3_2_homology_map_corrections.lp
  emdash3_2_chain_pair_cycle_factors.lp
  emdash3_2_homology_record_cycle_lifts.lp
  emdash3_2_homology_first_exactness.lp
)
reviewers=(
  examples/cokernel_record_covers.lp
  examples/homology_map_corrections.lp
  examples/homology_first_exactness.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-first-exactness-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-first-exactness.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-first-exactness.??????) ;;
...
printf 'homology-first-exactness check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-first-exactness checks succeeded; log: %s\n' "$log_file"
