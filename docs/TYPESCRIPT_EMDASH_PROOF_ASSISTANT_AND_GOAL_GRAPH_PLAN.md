...
| `D-PA-192` | Qualify the reviewed 12B3-R1 local workflow correction; pin its exact checkpoint in the immediately following ledger commit. | Exactly the npm workflow and its focused policy test change behavior: checkout action use is replaced by credential-empty exact public ref fetch, while release-only trigger, package steps, artifact handoff, protected OIDC publish, and every source byte remain unchanged. A fresh exact public Git probe, 3/3 release policy, workspace, root typecheck, focused lint, package build/preflight, YAML parse, no-secret/static denial, and diff hygiene pass. Actionlint and all long/kernel/book/browser/packed/sibling/hosted gates remain explicitly omitted. |
