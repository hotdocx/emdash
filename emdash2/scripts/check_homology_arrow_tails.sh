...
reviewers=(
  ...
  examples/computational_exact_arrow_tails.lp
  examples/homology_adjacent_window_tails.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-arrow-tails-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-arrow-tails.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-arrow-tails.??????) ;;
...
printf 'homology-arrow-tails check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-arrow-tails checks succeeded; log: %s\n' "$log_file"
