...
SCALE-STRESS-3 is in progress with declaration-only SCALE-STRESS-3A1 and the
two-part SCALE-STRESS-3A2 profunctor-action parent complete. 3A2A pins
`ProfMap` and comparison push/pull and closes the generic
nested-congruence retry exposed by identity-functor action. The correcting
SCALE-INTRINSIC-DELTA-1 boundary now checks transparent definitions attached
to existing Core owners, unfolds them without shadow declarations, and
rejects transparent dependency cycles. SCALE-STRESS-3A2B uses that generic
...
| PRODUCT-FACING-CONTINUATION | active in the dedicated product-demo plan; PRODUCT-DEMO-0A complete, D-DTTLF-PRODUCT-DEMO-001 separately recorded with human supersession, and PRODUCT-DEMO-1B final-green at `f1cb532a88ccca84786aa1cd5ee7cb006b1ad5fc` | completed usability/displayed evidence and SCALE-INDUCTIVE-HYBRID-0A; independent of the pending inductive closeout review | The root-only three-panel external-review CLI/guide and five focused tests pass with a 1,109-test aggregate and bounded kernel check; browser/parser/dependency/semantic delta is zero. Next measure the optional-oracle, browser, and user-syntax boundaries before selecting either implementation; no scale row or transfer claim is closed by presentation |
...
### SCALE-INDUCTIVE-1B1 Reviewed Implementation

`src/v3_2/lf_transfer_inductive_contract.ts` now supplies the approved generic
boundary. An explicit contract spec identifies a represented block, one
listed generated owner, the exact local beta IDs, and the expected
parameter/index/constructor counts. Association fails closed unless:

- exactly one source block lists the generated identity;
- exactly one absent-body, ordinary, opaque contract declaration has that
  identity and records the source block in `generatedBy`;
- the selected beta IDs exist, are all owned by that declaration, and exhaust
  its contract-local runtime rules;
- the represented block has the reviewed indexed shape; and
- no recursive occurrence appears in its head telescope, constructor fields,
  or result indices.
...
installed by a local declaration phase, and verifies the exact local beta
program. It contains no emdash owner names and remains outside the browser
barrel.

The active SCALE-STRESS-1A core representation is revision 2. Its `τΣ_`
block now has zero fixed parameters and the inline `a,P` binders as indices;
`Struct_sigma` explicitly rebinds them with its two implicit constructor
modes. The proposal retains a mechanically reconstructed legacy
signature-only view and proves that both representations lower to
structurally identical `τΣ_` and `Struct_sigma` declarations. The explicit
`ind_τΣ_` companion declaration, one beta, and first-projection witness now
route through the new association and generic compiler wrapper.

This completes the exact reviewed 1B1 transfer/integrity boundary. The
explicit `ind_τΣ_` declaration and beta are ordinary checked inputs needed by
the transferred consumer. The owner-generic association additionally proves
their provenance and exhaustiveness for this bounded nonrecursive indexed
case; 1B1 does not claim that this extra association is universally necessary
for importing every Lambdapi-generated owner. It adds no LF connective,
inferred backend-name trust, automatic eliminator synthesis, end-user
inductive-declaration facade, parser/surface syntax, active profile, browser
API, or Lambdapi source change.

Focused implementation validation on 2026-07-29:

```text
node --require ts-node/register --test --test-concurrency=1 \
  tests/v3_2_lf_transfer_inductive_contract_tests.ts \
  tests/v3_2_scale_inductive_1b_proposal_tests.ts \
  tests/v3_2_lf_transfer_inductive_tests.ts \
  tests/v3_2_scale_stress_1_representation_tests.ts
  31 tests: 30 passed, 1 intentional live skip, zero failures
...
./scripts/pnpmw run check:ts
  1,095 tests: 1,045 passed, 50 intentionally skipped, zero failures
  aggregate node:test duration: 1,169,124 ms
...
1. one ordinary explicit opaque declaration;
2. two ordinary subject-checked runtime rules; and
...
TypeScript-side positivity checking, automatic eliminator synthesis, or an
end-user `inductive` declaration API is required merely because Lambdapi
originally generated the symbol. Those remain deferred unless the explicit
...
## SCALE-MIXED-PHASE-1B Completion Record

`src/v3_2/lf_transfer_proof.ts` now provides one generic immutable
composition of source-separated proof programs. Every source phase retains
its exact compile artifact, module/source evidence, rule IDs, and phase-local
checking record. The executable view validates one pinned source and
dependency view, unique rule IDs, strict global source order, a resolvable
final declaration context, one immutable runtime-program identity, and one
explicit or already-agreed comparison budget before flattening the checked
rules.

The resulting `CoreLfComposedProofProgram` delegates to one
`CoreLfCompiledProofProgram`. Consequently all phases use one global
source-priority order, comparison queue, metavariable session, generated
constraint schedule, and step budget. Phase metadata separately records each
phase's rule IDs and all preceding proof-rule IDs; it does not rewrite the
original phase-local `checkedWithEarlierRuleIds` evidence to claim a stronger
compile-time dependency than was checked.

`src/v3_2/lf_transfer_mixed.ts` now returns both the exact per-phase
`proofPrograms` and the one executable `proofProgram`. Two proof phases
separated by declarations compose when they have the same runtime prefix.
The executable witness applies the later phase's rule at global rule index
one and reports that rule as the exact next action under a zero-step budget.
A runtime rule placed after both proof phases is explicitly absent from their
composed runtime, so the compiler does not silently widen them to the final
module runtime and proof rules never become evaluator rewrites.

This tranche intentionally rejects proof phases whose runtime-program
identities differ. Choosing final-runtime widening would give an earlier
proof rule later computation retroactively; independently executing each
phase would multiply queues, sessions, and budgets. `SCALE-MIXED-PHASE-1C`
therefore retains the exact runtime-divergent visibility audit/design. The
parent row remains in progress. The current J/Pi/Sigma/Nat stress corpus has
no proof rules, so its separate semantic-policy proposal remains
mechanically unblocked and still requires human approval.
...
node --require ts-node/register \
  ...
  20 tests / 2 suites: all passed

./scripts/pnpmw run check:ts
  workspace contract, typecheck, ESLint, and root tests passed
  507 tests / 63 suites: 479 passed, 28 process probes skipped
...
## SCALE-STRESS-2B2 Completion Record

SCALE-STRESS-2B2 extends the exact internal dependent-Pi runtime package
through its base-arrow action. Its checked acquisition contract pins the
smallest eight-command selection from `emdash2/emdash3_2.lp`: six symbols
and the two one-clause `fdapp1_int_cell` rules, under the same source and
canonical-export hashes as 2B1. The direct typed mixed module contains those
six declarations and two runtime phases; it reuses the exact 2B1 declaration
context and its completed 19-rule runtime by qualified identity.

`Fibre_cat` is installed from its checked transparent body. The exact
`functord_transport_lhs_func` and `functord_transport_rhs_func` bodies open a
separate `comp_cat_fapp0`/`catd_transport_func`/`Fibre_func` computation
closure, so this bounded slice preserves their checked types opaquely. Both
runtime clauses therefore carry exact self-invalidating subject-reduction
oracles for that measured omitted normalization closure. Their patterns,
plicities, ordered variables, right-hand sides, prior-rule lineages, and
execution are otherwise checked by the same owner-free generic engines used
by 2B1. A plicity-corrupted near miss remains irreducible.

The second clause's displayed-family fibre is represented by the exact
object type of `fapp0 (Op Cat_cat) Cat_cat Catd_cat_func (G x)`. This is the
active pullback-family computation at that point and avoids silently assuming
the deliberately omitted transparent body of `Catd_cat_func`. The live lane
checks canonical acquisition, the transparent `Fibre_cat` unfolding, both
base-arrow reductions, a well-typed non-conversion assertion, and the
corresponding rejected assertion.

This was a data-and-policy-only extension: no generic transfer, mixed-phase,
runtime, proof, or checker engine changed. It installs no active declaration,
...
node --require ts-node/register --test \
  ...
  30 tests / 4 suites: 26 passed, 4 live probes skipped

./scripts/pnpmw run check:scale-telescope-stress
  10 tests / 2 suites: 10 passed

./scripts/pnpmw run check:ts
  537 tests / 67 suites: 504 passed, 33 live probes skipped
...
## SCALE-CONVERSION-CONGRUENCE-1, SCALE-INTRINSIC-DELTA-1, And SCALE-STRESS-3A2 Completion Record
...
contains the transparent intrinsic `Hom` definition, eight new free
declarations, and the one identity-object runtime clause after the exact
SCALE-STRESS-3A1 declaration context and reviewed ten-rule continuation
runtime.
...
the backend-neutral `hom-classifier` Core owner. The initial 3A2A checkpoint
recorded a canonical fold, but the 3A2B product-hom audit showed that a fold
is insufficient: later generic conversion must be able to expose the exact
`Obj(Hom_cat(...))` body and then reduce `Hom_cat(Product_cat(...),...)`.
SCALE-INTRINSIC-DELTA-1 therefore extends the generic declaration boundary,
not the owner catalog:

- `checked-transparent-definition` may link to an existing `CoreOwnerId`;
- the transferred type must equal the intrinsic owner signature and the body
  is checked by the ordinary LF checker;
- the immutable LF environment stores the body separately from free
  declarations, so the semantic owner is never shadowed;
- delta reduction applies that body to the saturated owner arguments and
  continues through ordinary beta/congruence/runtime transitions under the
  existing shared budget;
- the complete transparent free/intrinsic dependency graph is checked and
  cycles are rejected, including cycles exposed when a previously opaque
  owner receives a later transfer equation; and
- mixed contexts permit only the exact refinement from the same
  intrinsic-conformance symbol/link to its transparent intrinsic definition.

The graph check is needed because independently checkpointed transfer slices
may already contain checked bodies that mentioned the then-opaque owner even
when the owner equation is source-prior in Lambdapi. This admits that
acyclic migration ordering without weakening the bounded evaluator or
allowing an arbitrary duplicate declaration. An unrelated
`opposite-category` fixture proves the compiler behavior, and self,
duplicate, ill-typed, foreign-environment, and cyclic cases fail closed.
...
With those mechanisms closed, all nine 3A2A source declarations check
through the shared mixed compiler. `ProfMap` delta-reduces to the existing
hom classifier, and that owner separately delta/beta-reduces to the exact
active `Obj(Hom_cat(...))` body;
...
SCALE-STRESS-3A2B then pins the exact eleven-command closure:
`sigma_Fst`, `sigma_Snd`, `Product_grpd`, its decode rule, `Product_cat`,
its object and hom-category rules, `Prof_tensor_map`, `Prof_tensor_func`,
and its object and capped-arrow rules. Six declarations remain opaque. The
five rules compile in exact source phases against the complete prior runtime
lineage. Product objects reduce through `Product_grpd` to the active decoded
constant-family dependent pair; product hom categories reduce componentwise;
and the two tensor-functor actions reduce to `Prof_tensor` and
`Prof_tensor_map` with the exact first/second projections.

All five new rule subjects check strictly without an oracle. In particular,
the capped-arrow subject follows the generic chain
`Hom` intrinsic delta → `Obj(Hom_cat(...))` → product hom-category runtime →
product object runtime → product groupoid decode. Focused execution observes
the product decode, both tensor actions, and a wrong-functor non-reduction.
The live lane pins all eleven canonical hashes and accepts the two active
Lambdapi actions while rejecting a changed arrow component.

The representation-only SCALE-STRESS-3A2 parent is complete. DefIso
cancellation, broader product functor/transfor closure, tensor
associativity/units, endpoint-changing cells, protected/evidence-module
behavior, WalkingEnd/HIT, batch throughput, and whole-transfer graduation
remain explicit non-claims. SCALE-STRESS-3B is the next dependency-ready
stress row.
...
```text
node --require ts-node/register --test \
  tests/v3_2_lf_definition_tests.ts \
  ...
  focused intrinsic/compiler/profunctor suites passed

./scripts/pnpmw run check:scale-profunctor-stress
  14 tests / 3 suites: 14 passed

./scripts/pnpmw run check:ts
  560 tests / 71 suites: 523 passed, 37 live probes skipped
...
- **2026-07-29 — SCALE-INDUCTIVE-1B1 implementation final-green; hybrid
  ...
  with zero erased-signature delta. One new owner-generic association/compiler
  boundary validates the explicit generated declaration, `generatedBy`, beta
  ownership, indexed shape, and zero recursive occurrences before delegating
  to the existing generic engines. The first-projection consumer reduces in
  TypeScript and Lambdapi. The final 1,095-test root gate and bounded active
...
- **2026-07-25 — SCALE-MIXED-PHASE-1B completed and checkpointed.** Recorded
  local implementation checkpoint
  `b7506a2d23c3c4ce5f9fdccd68cbdc9fe1f67d34` after 20 combined proof/mixed
  tests, the 507-test TypeScript gate, bounded active-kernel check, complete
  scale gate, and all live directed/acquisition probes passed. Separated
  proof phases sharing one exact runtime prefix now execute under one global
  rule priority, queue, metavariable session, generated-constraint schedule,
  and comparison budget while retaining their individual compile evidence.
  Later runtime rules are not silently visible. Runtime-divergent proof
...
- **2026-07-25 — SCALE-STRESS-2B1 internal/pullback Pi completed.** Pinned
  ...
  local implementation checkpoint:
  `568f1c9fa6aab6bb402c4c07d4190d15355cb2b7`.
- **2026-07-25 — SCALE-STRESS-2B2 Pi base-arrow action completed.** Pinned
  the exact eight-command selection and compiled six declarations plus two
  `fdapp1_int_cell` clauses after the exact 19-rule 2B1 lineage. Both clauses
  execute in TypeScript and agree with bounded live Lambdapi
  reduction/non-conversion evidence. `Fibre_cat` is transparent; the two
  transport bodies and their exact subject-normalization cases remain
  explicit opaque/oracle boundaries. No generic engine changed and no active
...
- **2026-07-25 — SCALE-STRESS-3A2A comparison action completed.** Pinned
  ten exact commands and compiled the active transparent `Hom` equation,
  eight free declarations, and the identity-object clause after the reviewed
  ...
  agree with bounded Lambdapi typing/rejection. The initial checkpoint
  represented `Hom` as a canonical owner fold; SCALE-INTRINSIC-DELTA-1
  subsequently strengthens that representation to a checked intrinsic owner
  body without changing its source command or semantic owner. Exact green
  local implementation checkpoint:
  `9d73a7a61659f3a47d1ae7e3eb448805b648572d`.
- **2026-07-25 — SCALE-INTRINSIC-DELTA-1 and SCALE-STRESS-3A2B
  completed.** The generic declaration environment/compiler now accepts an
  exact transparent definition linked to an existing Core owner, checks its
  intrinsic signature/body, rejects transparent dependency-graph cycles, and
  delta-unfolds saturated owner applications without a free shadow or
  owner-named engine case. Exact conformance-only-to-definition refinement is
  preserved across mixed contexts. The 3A2B slice pins six declarations and
  five rules for dependent-pair projections, products, and fixed-endpoint
  tensor action. Every subject checks without an oracle; product decode,
  tensor object action, and capped-arrow action execute in TypeScript and all
...
- **2026-07-29 — PRODUCT-DEMO-1B final-green.** After the separate exact
  D-DTTLF-PRODUCT-DEMO-001 review with human supersession, the implementation
  composes the existing outer dependent-LF, ordinary bracket, and genuine
  displayed-chain structured results into one root-only
  `demo:external-review` command and guide. Five focused tests, the
  1,109-test aggregate TypeScript gate, and bounded active-kernel check pass.
  No browser, parser/dependency, Lambdapi, generic-engine, elaborator, or
  mathematical owner/rule change occurred. Exact green local checkpoint:
