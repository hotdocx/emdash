...
| `AGENT-EVAL-12B4` | Reproducible real-agent runs and measured graduation | no-call refusal diagnosed; channel correction proposed but unauthorized | Closed no-model probes prove version is stdout-only while valid ChatGPT login status is stderr-only. The driver/fake modeled both as stdout. Dedicated R1 freezes a three-file exact-channel correction; the old ID/run root remain retired and no new call is authorized. |
