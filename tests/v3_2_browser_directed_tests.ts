...
describe('BROWSER-DIRECTED-1A additive browser entry', () => {
    ...
    it('keeps exact premise indexing Node-free and outside host adapters', () => {
        const closure = collectLocalClosure(
            'src/v3_2/lf_premise_index.ts'
        );
        assert.equal(
        ...
        assert.equal(
            closure.has(resolve('src/v3_2/lf_conversion.ts')),
            true
        );
        assert.equal(
            closure.has(resolve('src/v3_2/lf_transfer_acquisition.ts')),
    ...
    it('keeps selected-proof maintenance browser-safe and stateless', () => {
        const closure = collectLocalClosure(
            'src/v3_2/lf_proof_maintenance.ts'
        );
        assert.equal(
    ...
    it('keeps exact runtime-closure proof attachment Node-free', () => {
        const closure = collectLocalClosure(
    ...
    it('keeps declared-theorem dependency checking Node-free', () => {
        const closure = collectLocalClosure(
        ...
        assert.equal(
            closure.has(resolve('src/v3_2/lf_checker.ts')),
            true
        );
        assert.equal(
            closure.has(resolve('src/v3_2/ai_proof_cli.ts')),
    ...
    it('keeps fragment-module workspace authoring Node-free', () => {
        const closure = collectLocalClosure(
            'src/v3_2/lf_fragment_module_workspace_authoring.ts'
        );
        assert.equal(
            closure.has(resolve(
                'src/v3_2/lf_fragment_module_workspace_authoring.ts'
    ...
    it('keeps canonical research goal views browser-safe and host-free', () => {
        const closure = collectLocalClosure(
            'src/v3_2/research_goal_view.ts'
        );
        assert.equal(
            closure.has(resolve('src/v3_2/research_goal_view.ts')),
            true
        );
        assert.equal(
            closure.has(resolve('src/v3_2/research_goal_graph.ts')),
