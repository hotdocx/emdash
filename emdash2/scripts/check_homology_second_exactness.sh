...
reviewers=(
  examples/homology_epic_covers.lp
  examples/homology_second_exactness.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-second-exactness-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-second-exactness.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-second-exactness.??????) ;;
...
printf 'homology-second-exactness check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-second-exactness checks succeeded; log: %s\n' "$log_file"
