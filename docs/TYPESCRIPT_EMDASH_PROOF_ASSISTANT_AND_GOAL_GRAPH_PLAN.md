...
| `AGENT-EVAL-12B4` | Reproducible real-agent runs and measured graduation | R7 terminal fixed failure; R8 staged failure proposed | The sole classifier exited 1 with only its 41-byte fixed line, preserving privacy but yielding no stage. R7 cannot retry. R8 proposes only fixed failure-stage constants plus synthetic coverage. |
