...
prerequisites=(emdash3_2_homology_record_connecting.lp)
owners=(
  emdash3_2_hom_factor_zero_descent.lp
  emdash3_2_homology_record_cancellation.lp
  emdash3_2_homology_family_map_factors.lp
  emdash3_2_chain_pair_homology_actions.lp
  emdash3_2_homology_connecting_projection_zero.lp
  emdash3_2_homology_connecting_inclusion_zero.lp
  emdash3_2_homology_window.lp
)
reviewers=(
  examples/homology_family_map_factors.lp
  examples/chain_pair_homology_actions.lp
  examples/homology_window.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-window-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-window.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-window.??????) ;;
...
printf 'homology-window check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-window checks succeeded; log: %s\n' "$log_file"
