...
as discussed after implementing `homd_cov_int`, `tapp1_int_func_transf`, and `fapp1_int_transf`.
...
- `tapp1_int_func_transf` (outer index internalized via nested `Transf_cat`)

and a derived stable head:

- `fapp1_int_transf`, obtained by specializing `tapp1_int_func_transf` at the identity transfor.
...
1) **Probe restriction (simple, not a new “probe notion”)**
   ...
   - `E := Fibration_cov_catd E0` for some `E0 : Z ⟶ Cat_cat`.
   - Then `GG : Functord_cat (Fibration_cov_catd E0) D` matches the intended “computational probe into D”.
