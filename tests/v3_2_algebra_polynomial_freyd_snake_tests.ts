...
describe('v3.2 CAP-style polynomial Freyd snake morphism', () => {
    it('reproduces the CAP rational vector-space example without a section', () => {
        ...
        assert.equal(
            result.fiberProductIdentityFactor.reconstructionLeftAgreement.agrees,
            true
        );
        assert.equal(
            result.fiberProductIdentityFactor.reconstructionRightAgreement.agrees,
            true
        );
        assert.equal(result.p1Epimorphism.epic, true);
        assert.equal(result.pushout.compatibilityAgreement.agrees, true);
        assert.equal(
            result.pushoutIdentityCofactor.reconstructionLeftAgreement.agrees,
            true
        );
        assert.equal(
            result.pushoutIdentityCofactor.reconstructionRightAgreement.agrees,
            true
        );
