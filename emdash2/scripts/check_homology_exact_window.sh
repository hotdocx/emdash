...
reviewers=(
  examples/homology_exact_window.lp
)
mkdir -p logs/probes
log_file="$(pwd)/logs/probes/homology-exact-window-${mode}-$(date +%Y%m%d-%H%M%S).log"
stage_root="$(mktemp -d /tmp/emdash-homology-exact-window.XXXXXX)"
cleanup() {
  case "$stage_root" in
    /tmp/emdash-homology-exact-window.??????) ;;
...
printf 'homology-exact-window check log: %s\n' "$log_file"
cd "$stage_root"
...
printf 'homology-exact-window checks succeeded; log: %s\n' "$log_file"
