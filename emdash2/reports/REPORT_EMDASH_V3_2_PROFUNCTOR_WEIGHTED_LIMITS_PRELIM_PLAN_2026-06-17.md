...
| 4. Weighted limits | Ordinary `WeightedCone_prof`/`IsWeightedLimit_cov_iso`, ambient `Adjunction_hom_prof_iso_evidence`, genuinely defined `right_adjoint_preserves_weighted_limit_cov_iso`, computational `WeightedLimit_cov`, inverse universal/cone cells, the shaped adjunction/profunctor transpose bridge, and the full compatibility `right_adjoint_preserves_weighted_limit_cov` witness. | A coherent stronger computational representability classifier replacing the unsuffixed primitive witness, naturality in the shaped probe, explicit preserved-cone formula, and unit/counit component projections. |
